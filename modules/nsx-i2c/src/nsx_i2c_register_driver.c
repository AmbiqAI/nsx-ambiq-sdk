/**
 * @file nsx_i2c_register_driver.c
 * @author Ambiq
 ...
 */
uint32_t nsx_i2c_read_sequential_regs(
    nsx_i2c_config_t *cfg, uint32_t devAddr, uint32_t regAddr, void *buf, uint32_t size) {
 ...
 * @return uint32_t status
 */
uint32_t nsx_i2c_write_sequential_regs(
    nsx_i2c_config_t *cfg, uint32_t devAddr, uint32_t regAddr, void *buf, uint32_t size) {
    am_hal_iom_transfer_t Transaction;
    Transaction.ui8Priority = 1;
    Transaction.ui32InstrLen = 1;
 ...
 *
 * @param cfg Handle obtained from nsx_i2c_interface_init
 * @param devAddr Device address
 ...
 * @return uint32_t status
 */
uint32_t nsx_i2c_read_reg(
    nsx_i2c_config_t *cfg, uint32_t devAddr, uint8_t regAddr, uint8_t *value, uint8_t mask) {
    uint32_t regValue;
    if (nsx_i2c_read_sequential_regs(cfg, devAddr, regAddr, &regValue, 1)) {
        return NSX_I2C_STATUS_ERROR;
    }
    if (mask != 0xFF) {
...
uint32_t nsx_i2c_write_reg(
    ...
    if (nsx_i2c_write_sequential_regs(cfg, devAddr, regAddr, &regValue, 1)) {
        return NSX_I2C_STATUS_ERROR;
    }
    return NSX_I2C_STATUS_SUCCESS;
}
