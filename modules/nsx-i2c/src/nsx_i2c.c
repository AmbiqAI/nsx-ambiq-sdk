...
const nsx_core_api_t nsx_i2c_V0_0_1 = {.apiId = NSX_I2C_API_ID, .version = NSX_I2C_V0_0_1};
const nsx_core_api_t nsx_i2c_V1_0_0 = {.apiId = NSX_I2C_API_ID, .version = NSX_I2C_V1_0_0};
const nsx_core_api_t nsx_i2c_oldest_supported_version = {
    .apiId = NSX_I2C_API_ID, .version = NSX_I2C_V0_0_1};
const nsx_core_api_t nsx_i2c_current_version = {.apiId = NSX_I2C_API_ID, .version = NSX_I2C_V1_0_0};
...
uint32_t nsx_i2c_interface_init(nsx_i2c_config_t *cfg, uint32_t speed) {
    ...
    if (nsx_core_check_api(cfg->api, &nsx_i2c_oldest_supported_version, &nsx_i2c_current_version)) {
    ...
    if (am_hal_iom_initialize(cfg->iom, &(cfg->iomHandle)) ||
        ...
        return NSX_I2C_STATUS_ERROR;
    }

    // Configure the IOM pins.
    am_bsp_iom_pins_enable(cfg->iom, AM_HAL_IOM_I2C_MODE);
    return NSX_I2C_STATUS_SUCCESS;
}
...
uint32_t nsx_i2c_write_read(
    ...
    size_t numRead) {
    uint32_t st = nsx_i2c_write(cfg, writeBuf, numWrite, addr);
    if (st != NSX_I2C_STATUS_SUCCESS) return st;
    return nsx_i2c_read(cfg, readBuf, numRead, addr);
}
...
uint32_t nsx_i2c_transfer(nsx_i2c_config_t *cfg, nsx_i2c_msg_t *msgs, size_t numMsgs) {
    ...
    for (size_t i = 0; i < numMsgs; i++) {
        msg = &msgs[i];
        if (msg->flags == NSX_I2C_XFER_RD) {
            st = nsx_i2c_read(cfg, msg->buf, msg->len, msg->addr);
        } else if (msg->flags == NSX_I2C_XFER_WR) {
            st = nsx_i2c_write(cfg, msg->buf, msg->len, msg->addr);
        } else {
            continue;
        }
        if (st != NSX_I2C_STATUS_SUCCESS) return st;
