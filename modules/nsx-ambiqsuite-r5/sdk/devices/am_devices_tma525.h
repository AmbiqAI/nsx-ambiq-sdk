...
//*****************************************************************************
//
//! @brief Read raw data from the TMA525 touch sensor.
//!
//! This function reads `RxNumBytes` of raw touch data into `pui8RxBuffer`.
//! It performs the necessary I2C/IOM transactions to retrieve the data from
//! the sensor.
//!
//! @param pui8RxBuffer Pointer to the buffer that will receive raw data.
//! @param RxNumBytes   Number of bytes to read into the buffer.
//!
//! @return `AM_DEVICES_TMA525_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_TMA525_STATUS_ERROR` on failure.
//
//*****************************************************************************
...
//*****************************************************************************
//
//! @brief Retrieve the current touch point information (blocking).
//!
//! This function reads the current touch-point data from the TMA525 and
//! populates the supplied `touch_info` structure with the results.
//!
//! @param touch_info  Pointer to an `am_devices_tc_tma525_info_t` structure
//!                    that will be filled with touch data.
//!
//! @return `AM_DEVICES_TMA525_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_TMA525_STATUS_ERROR` on failure.
//
...
//*****************************************************************************
//
//! @brief Retrieve the current touch point information (non-blocking).
//!
//! This function starts a non-blocking read of touch information and fills
//! `touch_info` when data becomes available. Completion and synchronization are
//! managed by the underlying driver/IOM mechanisms.
//!
//! @param touch_info  Pointer to an `am_devices_tc_tma525_info_t` structure
//!                    that will be filled with touch data when available.
//!
//! @return `AM_DEVICES_TMA525_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_TMA525_STATUS_ERROR` on failure.
//
...
//*****************************************************************************
//
//! @brief Initialize the TMA525 touch driver.
//!
//! This function powers on and initializes the touch sensor and configures
//! the IOM/I2C interface. It also registers the optional `touch_handler`
//! interrupt callback and context pointer `pArg`.
//!
//! @param ui32Module     IOM module number to use for communication.
//! @param touch_handler  GPIO interrupt handler for touch events (may be NULL).
//! @param pArg           User context passed to the touch handler.
//!
//! @return `AM_DEVICES_TMA525_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_TMA525_STATUS_ERROR` on failure.
//
...
//*****************************************************************************
//
//! @brief De-initialize the TMA525 touch driver.
//!
//! This function disables the touch sensor and associated IOM resources for
//! the specified module.
//!
//! @param ui32Module  IOM module number associated with the touch sensor.
//!
//! @return `AM_DEVICES_TMA525_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_TMA525_STATUS_ERROR` on failure.
//
