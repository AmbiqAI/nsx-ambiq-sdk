...
//*****************************************************************************
//
//! @brief Get touch information from the CHSC5816 touch sensor.
//!
//! Reads the latest touch point(s) detected by the sensor and populates the
//! supplied `touch_event` structure with the results.
//!
//! @param touch_event - Pointer to an `am_devices_tc_chsc5816_info_t` structure
//!                      that will be filled with touch information.
//!
...
//*****************************************************************************
//
//! @brief Initialize the CHSC5816 device driver and IOM interface.
//!
//! Configures the driver and prepares the IOM instance for communication
//! with the CHSC5816 touch sensor.
//!
//! @param ui32Module    - IOM module number to use for the device.
//! @param touch_handler - GPIO interrupt handler for touch interrupt events.
//! @param pArg          - Optional argument passed to the `touch_handler`.
//!
//! @return 32-bit status: `AM_DEVICES_CHSC5816_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_CHSC5816_STATUS_ERROR` on failure.
//
...
//*****************************************************************************
//
//! @brief Non-blocking get touch information from the CHSC5816 sensor.
//!
//! Starts a non-blocking read of touch information and fills `touch_info`
//! when data becomes available.
//!
//! @param touch_info - Pointer to an `am_devices_tc_chsc5816_info_t` structure
//!                     to receive touch data.
//!
//! @return 32-bit status: `AM_DEVICES_CHSC5816_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_CHSC5816_STATUS_ERROR` on failure.
//
...
//*****************************************************************************
//
//! @brief Deinitialize the CHSC5816 device driver and release resources.
//!
//! Shuts down the driver and releases any IOM or GPIO resources associated
//! with the specified module.
//!
//! @param ui32Module - IOM module number used by the device.
//!
//! @return 32-bit status: `AM_DEVICES_CHSC5816_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_CHSC5816_STATUS_ERROR` on failure.
//
...
// #### INTERNAL BEGIN ####
extern int32_t
am_devices_chsc5816_fw_update(uint8_t fw_ver);
// #### INTERNAL END ####
