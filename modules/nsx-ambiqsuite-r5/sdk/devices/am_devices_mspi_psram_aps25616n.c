...
{
  {
    ...
    .bEnableFineDelay       = 0,
// #### INTERNAL BEGIN ####
#if defined(AM_PART_APOLLO330P_510L) && defined(APOLLO5_FPGA)
    .ui8TxDQSDelay          = 14,
    .ui8RxDQSDelay          = 22,
#else
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if !defined(APOLLO5_FPGA) // found drive strength affecting FPGA timing at low frequency
// #### INTERNAL END ####
...
//*****************************************************************************
//
//! @brief Handle am devices MSPI PSRAM aps25616n get wlc.
//!
//! Internal helper for the am devices MSPI PSRAM aps25616n get wlc operation.
//!
//! @param ui8WLC - Value for 8 WLC.
...
am_devices_mspi_psram_aps25616n_get_wlc(uint8_t ui8WLC, uint8_t *pWLC)
{
    am_devices_mspi_psram_aps25616n_wlc_e eWLC = ((am_devices_mspi_psram_aps25616n_wlc_e)(ui8WLC));
    switch(eWLC)
    {
...
am_devices_mspi_psram_aps25616n_verify_config(void *pHandle, uint8_t ui8RLC, uint8_t ui8WLC)
{
    ...
    else
    {
        ...
        am_util_debug_printf("PSRAM Register MR4 = 0x%02X\n", ui8RegValue);
        uint8_t ui8Temp;
        if (AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS != am_devices_mspi_psram_aps25616n_get_wlc((uint8_t)((ui8RegValue & 0xE0)>>5), &ui8Temp) )
        {
...
//*****************************************************************************
//
//! @brief Handle am devices MSPI PSRAM aps25616n config.
//!
//! Internal helper for the am devices MSPI PSRAM aps25616n config operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param pMSPISetting - Pointer to MSPI setting.
//!
//! @return Status code: `AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_PSRAM_STATUS_*`.
//
...
// #### INTERNAL BEGIN ####
    //
    // This driver is DDR and therefore is only for the APOLLO4 Family and up.
    //
// #### INTERNAL END ####
    ...
    mspiMemDevCfg->eCeLatency = AM_HAL_MSPI_CE_LATENCY_ADD1;
#endif
