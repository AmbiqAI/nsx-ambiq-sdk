...
//*****************************************************************************
//
//! @brief Get touch information from the CHSC5816 touch sensor (Apollo5).
//!
//! Reads touch data for the device identified by `pDevHandle` and fills the
//! provided `touch_info` structure.
//!
//! @param pDevHandle - Device handle returned by `am_devices_chsc5816_init`.
//! @param touch_info  - Pointer to an `am_devices_tc_chsc5816_info_t` structure
//!                     to be populated with touch information.
//!
//! @return 32-bit status: `AM_DEVICES_CHSC5816_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_CHSC5816_STATUS_ERROR` on failure.
//
//*****************************************************************************
extern uint32_t
...
//*****************************************************************************
//
//! @brief Start a non-blocking touch read operation.
//!
//! Initiates a non-blocking read for the device identified by `pDevHandle`.
//! When the operation completes, `pfnCallback` will be invoked with
//! `pCallbackCtxt` as its context (if `pfnCallback` is non-NULL).
//!
//! @param pDevHandle    - Device handle returned by `am_devices_chsc5816_init`.
//! @param touch_info    - Pointer to an `am_devices_tc_chsc5816_info_t` to
//!                      receive touch data when ready.
...
//*****************************************************************************
//
//! @brief Initialize the CHSC5816 device driver (Apollo5 variant).
//!
//! Sets up the IOM and device state for the CHSC5816 and returns handles.
//!
//! @param ui32Module     - IOM module number to use for the device.
//! @param pDevConfig     - Pointer to device configuration structure.
//! @param ppDevHandle    - Pointer to storage for returned device handle.
//! @param ppIomHandle    - Pointer to storage for returned IOM handle.
//! @param ui32IntPin     - GPIO number used for the touch interrupt.
//! @param ui32RstPin     - GPIO number used for the device reset line.
//! @param touch_handler  - GPIO interrupt handler for touch events.
//! @param pArg           - Argument passed to `touch_handler` when invoked.
//!
//! @return 32-bit status: `AM_DEVICES_CHSC5816_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_CHSC5816_STATUS_ERROR` on failure.
//
...
//*****************************************************************************
//
//! @brief Deinitialize the CHSC5816 device driver.
//!
//! Releases resources associated with the provided device handle.
//!
//! @param pDveHandle - Device handle returned by `am_devices_chsc5816_init`.
//!
//! @return 32-bit status: `AM_DEVICES_CHSC5816_STATUS_SUCCESS` on success,
//!         `AM_DEVICES_CHSC5816_STATUS_ERROR` on failure.
//
...
//*****************************************************************************
//
//! @brief Update the CHSC5816 firmware (internal use).
//!
//! Uploads firmware to the device and triggers any required reboot.
//!
//! @param pDveHandle - Device handle.
//! @param fw_ver     - Firmware version to program.
//!
//! @return signed 32-bit status (internal): 0 on success, negative on error.
//
//*****************************************************************************
extern int32_t
am_devices_chsc5816_fw_update(void *pDveHandle, uint8_t fw_ver);
// #### INTERNAL END ####
