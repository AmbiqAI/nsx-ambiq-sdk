...
typedef struct
{
    ...
    void                        *pIomHandle;
    bool                        bOccupied;
} am_devices_iom_mb85rq4ml_t;

am_devices_iom_mb85rq4ml_t gAmMb85rq4ml[AM_DEVICES_MB85RQ4ML_MAX_DEVICE_NUM];

am_hal_iom_config_t     g_sIomMb85rq4mlCfg =
{
...
//! @param ui32NumBytes
//!
//! @return am_devices_mb85rq4ml_status_t (status)
//
//*****************************************************************************
static uint32_t
                        ...
                        uint32_t *pData, uint32_t ui32NumBytes)
{
    ...
    am_devices_iom_mb85rq4ml_t *pIom = (am_devices_iom_mb85rq4ml_t *)pHandle;
    ...
    {
        return AM_DEVICES_MB85RQ4ML_STATUS_ERROR;
    }
    return AM_DEVICES_MB85RQ4ML_STATUS_SUCCESS;
}
...
//*****************************************************************************
//
// Initialize the mb85rq4ml driver.
//
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_init(uint32_t ui32Module, am_devices_mb85rq4ml_config_t *pDevConfig, void **ppHandle, void **ppIomHandle)
{
    void *pIomHandle;
    am_hal_iom_config_t     stIOMMB85RQ4MLSettings;
    ...
    for ( ui32Index = 0; ui32Index < AM_DEVICES_MB85RQ4ML_MAX_DEVICE_NUM; ui32Index++ )
    {
        if ( gAmMb85rq4ml[ui32Index].bOccupied == false )
        {
            break;
        }
    }
    if ( ui32Index == AM_DEVICES_MB85RQ4ML_MAX_DEVICE_NUM )
    {
        return AM_DEVICES_MB85RQ4ML_STATUS_ERROR;
    }
    ...
    stIOMMB85RQ4MLSettings = g_sIomMb85rq4mlCfg;
    stIOMMB85RQ4MLSettings.ui32NBTxnBufLength = pDevConfig->ui32NBTxnBufLength;
    stIOMMB85RQ4MLSettings.pNBTxnBuf = pDevConfig->pNBTxnBuf;
    stIOMMB85RQ4MLSettings.ui32ClockFreq = pDevConfig->ui32ClockFreq;
    ...
    if (am_hal_iom_initialize(ui32Module, &pIomHandle) ||
        ...
        am_hal_iom_configure(pIomHandle, &stIOMMB85RQ4MLSettings) ||
    ...
    {
        gAmMb85rq4ml[ui32Index].bOccupied = true;
        gAmMb85rq4ml[ui32Index].ui32CS = g_CS[ui32Module];
        gAmMb85rq4ml[ui32Index].ui32Module = ui32Module;
        *ppIomHandle = gAmMb85rq4ml[ui32Index].pIomHandle = pIomHandle;
        *ppHandle = (void *)&gAmMb85rq4ml[ui32Index];
        //
        // Return the status.
        //
        return AM_DEVICES_MB85RQ4ML_STATUS_SUCCESS;
...
//*****************************************************************************
//
//  De-Initialize the mb85rq4ml driver.
//
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_term(void *pHandle)
{
    am_devices_iom_mb85rq4ml_t *pIom = (am_devices_iom_mb85rq4ml_t *)pHandle;
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_read_id(void *pHandle, uint32_t *pDeviceID)
{
    //
    // Send the command sequence to read the Device ID.
    //
    if (am_device_command_read(pHandle, 1, AM_DEVICES_MB85RQ4ML_READ_DEVICE_ID, pDeviceID, 4))
    {
        return AM_DEVICES_MB85RQ4ML_STATUS_ERROR;
    }
    return AM_DEVICES_MB85RQ4ML_STATUS_SUCCESS;
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_status_get(void *pHandle, uint32_t *pStatus)
{
    //
    // Send the command sequence to read the device status.
    //
    if (am_device_command_read(pHandle, 1, AM_DEVICES_MB85RQ4ML_READ_STATUS, pStatus, 1))
    {
        return AM_DEVICES_MB85RQ4ML_STATUS_ERROR;
    }
    return AM_DEVICES_MB85RQ4ML_STATUS_SUCCESS;
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_command_send(void *pHandle, uint32_t ui32Cmd)
{
    uint32_t Dummy;

    if ( ui32Cmd == AM_DEVICES_MB85RQ4ML_CMD_WREN )
    {
        //
        // Send the command to enable writing.
        //
        if (am_device_command_write(pHandle, 1, AM_DEVICES_MB85RQ4ML_WRITE_ENABLE, &Dummy, 0))
        {
            return AM_DEVICES_MB85RQ4ML_STATUS_ERROR;
        }

    }
    else if ( ui32Cmd == AM_DEVICES_MB85RQ4ML_CMD_WRDI )
    {
        //
        // Send the command to enable writing.
        //
        if (am_device_command_write(pHandle, 1, AM_DEVICES_MB85RQ4ML_WRITE_DISABLE, &Dummy, 0))
        {
            return AM_DEVICES_MB85RQ4ML_STATUS_ERROR;
        }
    }
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_blocking_write(void *pHandle, uint8_t *pui8TxBuffer,
                                    ...
                                    uint32_t ui32NumBytes)
{
    ...
    Transaction.ui32Instr       = AM_DEVICES_MB85RQ4ML_WRITE;
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_nonblocking_write_adv(void *pHandle, uint8_t *pui8TxBuffer,
                                           uint32_t ui32WriteAddress,
                                           uint32_t ui32NumBytes,
                                           ...
                                           void *pCallbackCtxt)
{
    ...
    Transaction.ui32Instr       = AM_DEVICES_MB85RQ4ML_WRITE_ENABLE;
    ...
    // No Need to issue AM_DEVICES_MB85RQ4ML_WRITE_DISABLE, as WEL gets deasserted
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_nonblocking_write(void *pHandle, uint8_t *pui8TxBuffer,
                                       uint32_t ui32WriteAddress,
                                       uint32_t ui32NumBytes,
                                       am_hal_iom_callback_t pfnCallback,
                                       void *pCallbackCtxt)
{
    ...
    //
    Transaction.ui8Priority     = 1;        // High priority for now.
    Transaction.uPeerInfo.ui32SpiChipSelect = g_MB85RQ4MLCS;
    Transaction.ui8RepeatCount  = 0;
    Transaction.ui32PauseCondition       = 0;
    Transaction.ui32StatusSetClr = 0;
    ...
    if (am_devices_mb85rq4ml_nonblocking_write_adv(pui8TxBuffer,
                                                   ui32WriteAddress,
                                                   ui32NumBytes,
                                                   0, 0, 0, 0))
    {
    ...
    Transaction.eDirection      = AM_HAL_IOM_TX;
    Transaction.ui32InstrLen    = 1;        // Sending 1 offset byte
    Transaction.ui32Instr       = AM_DEVICES_MB85RQ4ML_WRITE_DISABLE;
    ...
    return AM_DEVICES_MB85RQ4ML_STATUS_SUCCESS;
#else
    return (am_devices_mb85rq4ml_nonblocking_write_adv(pHandle, pui8TxBuffer,
                                                   ui32WriteAddress,
                                                   ui32NumBytes,
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_blocking_read(void *pHandle, uint8_t *pui8RxBuffer,
                                   ...
                                   uint32_t ui32NumBytes)
{
    ...
    Transaction.eDirection      = AM_HAL_IOM_TX;
    Transaction.ui32InstrLen    = 1;
    Transaction.ui32Instr       = AM_DEVICES_MB85RQ4ML_FAST_READ;
    Transaction.ui32NumBytes    = 0;
...
//*****************************************************************************
uint32_t
am_devices_mb85rq4ml_nonblocking_read(void *pHandle, uint8_t *pui8RxBuffer,
                                      uint32_t ui32ReadAddress,
                                      uint32_t ui32NumBytes,
                                      am_hal_iom_callback_t pfnCallback,
                                      void *pCallbackCtxt)
