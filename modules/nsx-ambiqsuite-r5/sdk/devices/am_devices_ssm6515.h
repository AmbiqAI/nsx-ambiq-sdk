...
//*****************************************************************************
//
//! @brief Select SSM6515 TDM slot to route audio data to DAC
//!
//! @note There would be several TDM slots(channels) in one TDM frame, this
//! function is used to select one slot to route audio data to DAC. One SSM6515
//! chip can only route one slot to DAC, if multiple SSM6515 chips are used in
//! TDM mode, different chip should select different slot number according to
//! the actual TDM data distribution.
//!
//! @param chip_addr - Chip I2C device address.
//!
//! @param slot - TDM slot number, valid value is 0~31.
//!
//! @return status - Generic or interface specific status.
//*****************************************************************************
extern uint32_t am_devices_ssm6515_select_tdm_slot(uint8_t chip_addr, uint32_t slot);
