...
//*****************************************************************************
//
//! @brief Handle am devices MSPI rm69330 command write.
//!
//! Internal helper for the am devices MSPI rm69330 command write operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param ui32Instr - Value for 32 instr.
//! @param pData - Data buffer pointer.
//! @param ui32NumBytes - Value for 32 num bytes.
//!
//! @return Status code from `AM_DEVICES_MSPI_RM69330_STATUS_*`.
//
//*****************************************************************************
static uint32_t
...
//*****************************************************************************
//
//! @brief Handle am devices MSPI rm69330 command read.
//!
//! Internal helper for the am devices MSPI rm69330 command read operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param ui32Instr - Value for 32 instr.
//! @param pData - Data buffer pointer.
...
//*****************************************************************************
//
//! @brief Handle am devices set row col.
//!
//! Internal helper for the am devices set row col operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param psGraphic_conf - Pointer to psgraphic conf.
//!
...
//*****************************************************************************
//
//! @brief Handle am devices LCM init.
//!
//! Internal helper for the am devices LCM init operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param psGraphic_conf - Pointer to psgraphic conf.
//!
//! @return Status code: `AM_DEVICES_MSPI_RM69330_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_RM69330_STATUS_*`.
//
//*****************************************************************************
static uint32_t
...
// #### INTERNAL BEGIN ####
    ...
    //
    if ( g_sDispCfg.bTEAdj )
    {
        const int MIPI_set_cmd_page = 0xFE;
        //
        // Manufacture Command Set
        //
        ui8CMDBuf[0] = 0x01;
        if (am_devices_mspi_rm69330_command_write(pHandle, MIPI_set_cmd_page, ui8CMDBuf, 1))
        {
            return AM_DEVICES_MSPI_RM69330_STATUS_ERROR;
        }
        ...
        am_util_delay_ms(10);
        //
        // setting frequency to 45.0Hz,this parameter may be vary depending on panel vedors.
        //
        ui8CMDBuf[0] = 0xA0;
        if (am_devices_mspi_rm69330_command_write(pHandle, 0x28, ui8CMDBuf, 1))
        {
            return AM_DEVICES_MSPI_RM69330_STATUS_ERROR;
        }
