...
am_hal_mspi_dev_config_t  APMDDROctalMSPIConfig =
{
  ...
  .bEmulateDDR          = true,
#if defined(AM_PART_APOLLO5_API)
  .eCeLatency           = AM_HAL_MSPI_CE_LATENCY_ADD1,
#endif
// #### INTERNAL BEGIN ####
...
//*****************************************************************************
//
//! @brief Handle am devices PSRAM aps25616ba default regs set.
//!
//! Internal helper for the am devices PSRAM aps25616ba default regs set operation.
//!
//! @param pPsram - Pointer to PSRAM.
//!
//! @return Status code: `AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_PSRAM_STATUS_*`.
//
//*****************************************************************************
static uint32_t
...
#if defined(USE_APS51216BA) || defined(USE_APS51216CA_E9)
...
#if defined(USE_APS51216CA_E9)
    pPsram->sRegisters.MR4_b.WLC = AM_DEVICES_MSPI_PSRAM_APS_Z8_WLC_9;
#else
...
//*****************************************************************************
//
//! @brief Handle am devices MSPI PSRAM aps z8 get wlc.
//!
//! Internal helper for the am devices MSPI PSRAM aps z8 get wlc operation.
//!
//! @param eWLC - Value for e WLC.
//! @param pWLC - Pointer to WLC.
//!
//! @return Status code: `AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_PSRAM_STATUS_*`.
//
...
//*****************************************************************************
//
//! @brief Handle pfnmspi APMPSRAM DDR callback.
//!
//! Internal helper for the pfnmspi APMPSRAM DDR callback operation.
//!
//! @param pCallbackCtxt - Pointer to callback ctxt.
...
// #### INTERNAL BEGIN ####
//
// Can be removed once APS51216DA_E10 does support software reset
//
#if defined(USE_APS51216DA_E10)
  am_util_debug_printf("******************** send reset **********************\n");
...
am_devices_mspi_psram_aps25616ba_info(am_devices_mspi_psram_t *pPsram)
{
  ...
  // #### INTERNAL BEGIN ####
  //
  // read all registers and print out
  //
  am_util_debug_printf("Read PSRAM MR[0]\n");
  for (uint32_t i = 0; i < 10; i++)
  {
    if (AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS != am_device_command_read(pPsram->pMspiHandle, AM_DEVICES_MSPI_PSRAM_DDR_READ_REGISTER, true, i, &ui32Rawdata, 4))
    {
        am_util_debug_printf("Read PSRAM MR[%d]!\n", i);
        return AM_DEVICES_MSPI_PSRAM_STATUS_ERROR;
    }
    else
    {
        am_util_debug_printf("PSRAM Register MR[%d] = 0x%02X\n", i, (uint8_t)ui32Rawdata);
    }
  }
  // #### INTERNAL END ####
  //
...
#else
    am_util_debug_printf("PSRAM RBX Supported by non-configured default\n\n");
#endif
  }
...
//*****************************************************************************
//
//! @brief Handle am devices MSPI peripheral init.
//!
//! Internal helper for the am devices MSPI peripheral init operation.
//!
//! @param ui32Module - Value for 32 module.
//! @param pDevCfg - Pointer to configuration data.
//! @param pPsram - Pointer to PSRAM.
//!
//! @return Status code: `AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_PSRAM_STATUS_*`.
...
#if defined(USE_APS51216CA_E9)
  if (AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS != am_devices_mspi_psram_aps_z8_get_wlc(AM_DEVICES_MSPI_PSRAM_APS_Z8_WLC_7, &pPsram->stSetting.ui8WriteLatency) )
#else
...
//*****************************************************************************
//
//! @brief Handle am devices MSPI PSRAM aps25616ba device init.
//!
//! Internal helper for the am devices MSPI PSRAM aps25616ba device init operation.
//!
//! @param pPsram - Pointer to PSRAM.
//! @param eTargetFreq - Value for e target freq.
//!
//! @return Status code: `AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_PSRAM_STATUS_*`.
//
...
if (AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS != am_device_command_read(pPsram->pMspiHandle, AM_DEVICES_MSPI_PSRAM_DDR_READ_REGISTER, true, 9, &ui32Rawdata, 4))
  {
      am_util_debug_printf("Failed to read PSRAM MR8 register!\n");
      return AM_DEVICES_MSPI_PSRAM_STATUS_ERROR;
  }
  ...
  {
      pPsram->sRegisters.MR9 = (uint8_t)ui32Rawdata;
      am_util_debug_printf("PSRAM Register MR9 = 0x%02X\n", pPsram->sRegisters.MR9);
      am_util_debug_printf("PSRAM MR9[3] = 0x%X\n", pPsram->sRegisters.MR9_b.REF_IND);

      if (pPsram->sRegisters.MR9_b.REF_IND)
      {
        ui32Rawdata = 0;
        if (AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS != am_device_command_write(pPsram->pMspiHandle, AM_DEVICES_MSPI_PSRAM_DDR_WRITE_REGISTER, true, 9, &ui32Rawdata, 1))
        {
          am_util_debug_printf("Failed to write PSRAM Register MR9!\n");
          return AM_DEVICES_MSPI_PSRAM_STATUS_ERROR;
        }
        else
        {
...
#elif defined (USE_APS51216CA_E9)
  switch(eTargetFreq)
  {
#if defined (AM_PART_APOLLO5_API)
    case AM_HAL_MSPI_CLK_250MHZ:
...
//*****************************************************************************
//
//! @brief Handle MSPI aps25616ba ddr DMA read.
//!
//! Internal helper for the MSPI aps25616ba ddr DMA read operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param pui8RxBuffer - Data buffer pointer.
//! @param ui32ReadAddress - Address value.
...
#endif
static uint32_t
mspi_aps25616ba_ddr_dma_read(void *pHandle, uint8_t *pui8RxBuffer,
                             uint32_t ui32ReadAddress,
                             uint32_t ui32NumBytes)
{
  uint32_t                      ui32Status;
...
//*****************************************************************************
//
//! @brief Handle MSPI aps25616ba ddr DMA write.
//!
//! Internal helper for the MSPI aps25616ba ddr DMA write operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param pui8TxBuffer - Data buffer pointer.
//! @param ui32WriteAddress - Address value.
//! @param ui32NumBytes - Value for 32 num bytes.
//!
//! @return Status code: `AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_PSRAM_STATUS_*`.
...
#endif
static uint32_t
mspi_aps25616ba_ddr_dma_write(void *pHandle, uint8_t *pui8TxBuffer,
                              uint32_t ui32WriteAddress,
                              uint32_t ui32NumBytes)
{
    uint32_t                      ui32Status;
...
//*****************************************************************************
//
//! @brief Prepare test pattern.
//!
//! Internal helper for the prepare test pattern operation.
//!
//! @param pattern_index - Index value.
//! @param buff - Data buffer pointer.
//! @param len - Length/count for len.
//!
//! @return Status or signed result value.
//
//*****************************************************************************
...
//*****************************************************************************
//
//! @brief Handle PSRAM check by XIP.
//!
//! Internal helper for the PSRAM check by XIP operation.
//!
//! @param length - Length/count for length.
