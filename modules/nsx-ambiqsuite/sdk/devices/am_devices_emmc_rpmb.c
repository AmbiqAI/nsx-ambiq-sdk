...
am_devices_emmc_mebedtls_init(void)
{
    ...
    {
        AM_DEVICE_RPMB_DEBUG("Error during read of OTP power status\n");
    }
    if (!bOTPEnabled)
    {
        ...
        if (AM_HAL_STATUS_SUCCESS != ui32Status)
        {
            AM_DEVICE_RPMB_DEBUG("Error in power up of OTP module\n");
        }
    }
#endif
    //
    ...
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        AM_DEVICE_RPMB_DEBUG("Error in power up of Crypto module\n");
    }
...
am_devices_emmc_mebedtls_deinit(void)
{
    ...
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        AM_DEVICE_RPMB_DEBUG("Error in power down of Crypto module\n");
    }
#if defined (AM_PART_APOLLO5_API)
    //
    ...
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        AM_DEVICE_RPMB_DEBUG("Error in power down of OTP module\n");
    }
#endif
    AM_DEVICE_RPMB_DEBUG("Crypto powered down!\n");
