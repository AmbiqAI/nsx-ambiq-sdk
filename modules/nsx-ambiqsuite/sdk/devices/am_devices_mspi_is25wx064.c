...
// #### INTERNAL BEGIN ####
// CAYNSWS-1297 Reduce timing scan time for IS25WX064 Octal Flash
// #### INTERNAL END ####
...
// ##### INTERNAL BEGIN #####
static uint32_t
is25wx064_verify_setup(void *pHandle);
// ##### INTERNAL END #####
...
#if 0
static void
is25wx064_data_patten_init(void *pHandle);
#endif
// ##### INTERNAL END #####
...
//*****************************************************************************
//
//! @brief Handle is25wx064 data patten init.
//!
//! Internal helper for the is25wx064 data patten init operation.
//!
...
is25wx064_data_patten_init(void *pHandle)
{
    static uint8_t ui8Send  = 0x40;
    uint8_t ui8Response     = 0;

    ui8Send++;
    command_write_serial(pHandle, AM_DEVICES_MSPI_IS25WX064_WRITE_ENABLE, false, 0, (uint32_t *)NULL, 0);
    //am_util_delay_ms(1);
    command_write_serial(pHandle, AM_DEVICES_MSPI_IS25WX064_WRITE_NONVOLATILE_CR, true, 0x0A, (uint32_t *)&ui8Send, 1);
    am_util_stdio_printf("set DLP patten = 0x%02x\n", ui8Send);
    am_util_delay_ms(1);
    is25wx064_busy_wait_until(pHandle, 10);
    command_read_serial(pHandle, AM_DEVICES_MSPI_IS25WX064_READ_NONVOLATILE_CR, true, 0x0A, (uint32_t *)&ui8Response, 1);
    am_util_delay_ms(1);
    is25wx064_busy_wait_until(pHandle, 10);
    am_util_stdio_printf("read DLP patten = 0x%02x\n", ui8Response);
...
//! @param timeout_ms
//!
//! @return Status code from `AM_DEVICES_MSPI_IS25WX064_STATUS_*`.
//*****************************************************************************
static am_devices_mspi_is25wx064_status_t
is25wx064_busy_wait_until(void *pHandle, uint32_t timeout_ms)
{
...
//*****************************************************************************
//
//! @brief Handle is25wx064 flag status register read until.
//!
//! Internal helper for the is25wx064 flag status register read until operation.
//!
//! @param pHandle - Pointer to the device handle.
...
//! @param dev_config
//!
//! @return Status code: `AM_DEVICES_MSPI_IS25WX064_STATUS_SUCCESS` on success;
//!         otherwise `AM_DEVICES_MSPI_IS25WX064_STATUS_*`.
//
//*****************************************************************************
static uint32_t
is25wx064_pre_config(void *pHandle, am_hal_mspi_dev_config_t *dev_config)
...
is25wx064_verify_setup(void *pHandle)
{
    ...
    switch ( pFlash->stSetting.eDeviceConfig )
    {
        ...
        case AM_HAL_MSPI_FLASH_OCTAL_CE0_1_8_8:
        case AM_HAL_MSPI_FLASH_OCTAL_CE1_1_8_8:
            ui32Status = command_read_serial(pHandle, AM_DEVICES_MSPI_IS25WX064_READ_VOLATILE_CR, true, 0x00, (uint32_t *)&ui8Response, 1);
            am_util_stdio_printf("Serial mode read CR[0] = 0x%02x\n", ui8Response);
            ui32Status = command_read_serial(pHandle, AM_DEVICES_MSPI_IS25WX064_READ_VOLATILE_CR, true, 0x01, (uint32_t *)&ui8Response, 1);
            am_util_stdio_printf("Serial mode read CR[1] = 0x%02x\n", ui8Response);
            ui32Status = command_read_serial(pHandle, AM_DEVICES_MSPI_IS25WX064_READ_VOLATILE_CR, true, 0x06, (uint32_t *)&ui8Response, 1);
            am_util_stdio_printf("Serial mode read CR[6] = 0x%02x\n", ui8Response);
            break;
        case AM_HAL_MSPI_FLASH_OCTAL_DDR_CE0:
        case AM_HAL_MSPI_FLASH_OCTAL_DDR_CE1:
            ui32Status = command_read_octal(pHandle, AM_DEVICES_MSPI_IS25WX064_OCTA_READ_V_CFG_REG_CMD, true, 0x0000, (uint32_t *)ui8Response_array, sizeof(ui8Response_array));
            am_util_stdio_printf("Octal DDR mode read CR[0] = 0x%02x\n", ui8Response_array[0]);
            break;
        default:
...
//!
//! Internal helper for the enter command mode operation.
//!
//! @param pHandle - Pointer to the device handle.
//!
//! @return Status code: `AM_DEVICES_MSPI_IS25WX064_STATUS_SUCCESS` on success;
...
//!
//! Internal helper for the exit command mode operation.
//!
//! @param pHandle - Pointer to the device handle.
//!
//! @return Status code: `AM_DEVICES_MSPI_IS25WX064_STATUS_SUCCESS` on success;
...
//! @param ui32NumBytes
//!
//! @return Status code from `AM_HAL_STATUS_*`.
//
//*****************************************************************************
static uint32_t
command_write_combo(void *pHandle,
...
//*****************************************************************************
//
//! @brief Handle command read combo.
//!
//! Internal helper for the command read combo operation.
//!
//! @param pHandle - Pointer to the device handle.
//! @param ui16Instr - Value for 16 instr.
                               ...
                               void **ppMspiHandle)
{
    ...
    // ##### INTERNAL BEGIN #####
    // is25wx064_verify_setup((void *)&gAmIS25WX064[ui32Index]);
    // ##### INTERNAL END #####
...
//*****************************************************************************
uint32_t
am_devices_mspi_is25wx064_read_cb(void *pHandle,
                                  uint8_t *pui8RxBuffer,
                                  uint32_t ui32ReadAddress,
                                  uint32_t ui32NumBytes,
                                  am_hal_mspi_callback_t pfnCallback,
...
//*****************************************************************************
uint32_t
am_devices_mspi_is25wx064_read_callback(void *pHandle,
                                        uint8_t *pui8RxBuffer,
                                        uint32_t ui32ReadAddress,
                                        uint32_t ui32NumBytes)
{
    ...
    ui32Status = am_hal_mspi_nonblocking_transfer(pFlash->pMspiHandle, &Transaction,
                                                  ...
                                                  pfnMSPI_IS25WX064_Callback, (void *)&ui32DMAStatus);
    //
    // Check the transaction status.
    //
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
...
//!  @note Generate the test data patterns.\n
...
//*****************************************************************************
//! @brief verifies the data pattern in the echo-with-inversion way
//!
//! @param pattern
//! @param buffer
//! @param length
...
//*****************************************************************************
//! @brief  checks the timing parameters set for octal communication
//!
//! @param pFlashHandle
//! @param length
//! @return true true if verify failure happens,
...
// #### INTERNAL BEGIN ####
//
// The new function am_hal_mspi_blocking_transfer_continuous() was added for the Apollo4 series.
// So We have to comment it out for Apollo5 or later.
//
// #### INTERNAL END ####
#if defined(AM_PART_APOLLO4_API)
uint32_t
