...
#define AM_DEVICES_COOPER_SBL_CHECK_TIMES_MINIMUM 200
...
am_devices_cooper_init(uint32_t ui32Module, am_devices_cooper_config_t* pDevConfig, void** ppHandle, void** ppBleHandle)
{
    ...
    am_hal_iom_config_t     stIOMCOOPERSettings = {0};
    ...
    else
    {
        ...
        //
        ui32ErrorStatus = am_hal_mcuctrl_control(AM_HAL_MCUCTRL_CONTROL_EXTCLK32K_ENABLE, NULL);
        if ( ui32ErrorStatus )
        {
            return AM_DEVICES_COOPER_STATUS_ERROR;
        }
...
// #### INTERNAL END ####
    ...
    do
    {
        ...
        // Regarding the situation where some chips may receive an interrupt between approximately 100ms and 200ms, ignore this interrupt.
        // The measurement value of the period of Cooper SBL check and SPI initialization
        // is 400ms ~ 500ms
        if ((ui32IntStatus && (ui32WaitReadyCount >= AM_DEVICES_COOPER_SBL_CHECK_TIMES_MINIMUM)) ||
            (ui32WaitReadyCount >= AM_DEVICES_COOPER_SBL_CHECK_TIMES))
        {
            break;
        }
        am_util_delay_ms(1);
