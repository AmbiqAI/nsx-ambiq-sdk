//*****************************************************************************
//
//! @file am_devices_mspi_w25n02kw.c
//!
...
#define W25N02KW_QUAD_CLKON4_MODE_EN
#endif
// #### INTERNAL BEGIN ####
//
//! @note W25N02KW_QUAD_CLKON4_MODE_EN is currently only tested for the apollo4l_eb
//
// #### INTERNAL END ####

#define AM_DEVICES_MSPI_W25N02KW_TIMEOUT       1000000
#define AM_DEVICES_MSPI_W25N02KW_ERASE_TIMEOUT 1000000
...
static am_hal_mspi_config_t gMspiCfg =
{
  .ui32TCBSize          = 0,
  .pTCB                 = NULL,
#if defined(W25N02KW_QUAD_CLKON4_MODE_EN)
  .bClkonD4             = 1
#else
  .bClkonD4             = 0
#endif
...
static am_devices_mspi_w25n02kw_t gAmW25n02kw[AM_DEVICES_MSPI_W25N02KW_MAX_DEVICE_NUM];

static am_hal_mspi_dev_config_t MSPI_W25N02KW_Serial_CE0_MSPIConfig =
{
    ...
    .bTurnaround = true,
#if defined(AM_PART_APOLLO4_API) || defined(AM_PART_APOLLO5_API)
    .ui16ReadInstr         = AM_DEVICES_MSPI_W25N02KW_READ_BUFFER_X1,
    .ui16WriteInstr        = AM_DEVICES_MSPI_W25N02KW_PROGRAM_LOAD_X1,
#else
    .ui8ReadInstr         = AM_DEVICES_MSPI_W25N02KW_READ_BUFFER_X1,
    .ui8WriteInstr        = AM_DEVICES_MSPI_W25N02KW_PROGRAM_LOAD_X1,
#endif
...
static am_hal_mspi_dev_config_t MSPI_W25N02KW_Serial_CE1_MSPIConfig =
{
    .eSpiMode = AM_HAL_MSPI_SPI_MODE_0,
    .eClockFreq = AM_HAL_MSPI_CLK_4MHZ,
    .ui8TurnAround = 8,
...
static am_hal_mspi_dev_config_t MSPI_W25N02KW_Quad_CE0_MSPIConfig =
{
    .eSpiMode = AM_HAL_MSPI_SPI_MODE_0,
    .eClockFreq = AM_HAL_MSPI_CLK_4MHZ,
    .ui8TurnAround = 8,
    ...
    .bTurnaround = true,
#if defined(AM_PART_APOLLO4_API) || defined(AM_PART_APOLLO5_API)
    .ui16ReadInstr         = AM_DEVICES_MSPI_W25N02KW_READ_BUFFER_X4,
    .ui16WriteInstr        = AM_DEVICES_MSPI_W25N02KW_PROGRAM_LOAD_X4,
#else
    .ui8ReadInstr         = AM_DEVICES_MSPI_W25N02KW_READ_BUFFER_X4,
    .ui8WriteInstr        = AM_DEVICES_MSPI_W25N02KW_PROGRAM_LOAD_X4,
#endif
...
static am_hal_mspi_dev_config_t MSPI_W25N02KW_Quad_CE1_MSPIConfig =
{
    .eSpiMode = AM_HAL_MSPI_SPI_MODE_0,
    .eClockFreq = AM_HAL_MSPI_CLK_4MHZ,
    .ui8TurnAround = 8,
...
} g_W25N02KW_DevConfig[] =
{
    { AM_HAL_MSPI_FLASH_SERIAL_CE0, &MSPI_W25N02KW_Serial_CE0_MSPIConfig },
    { AM_HAL_MSPI_FLASH_SERIAL_CE1, &MSPI_W25N02KW_Serial_CE1_MSPIConfig },
    { AM_HAL_MSPI_FLASH_QUAD_CE0_1_1_4,   &MSPI_W25N02KW_Quad_CE0_MSPIConfig   },
    { AM_HAL_MSPI_FLASH_QUAD_CE1_1_1_4,   &MSPI_W25N02KW_Quad_CE1_MSPIConfig   },
};
...
static am_devices_mspi_w25n02kw_sdr_timing_config_t SDRTimingConfigDefault =
{
    .ui32Turnaround     = 9,
    .ui32Rxneg          = 0,
    .ui32Rxdqsdelay     = 15
...
//! SDR timing stored setting
//
static am_devices_mspi_w25n02kw_sdr_timing_config_t SDRTimingConfigStored;
#endif // defined(AM_PART_APOLLO4_API)

#if defined (AM_PART_APOLLO3P)
static uint8_t PageBuffer[AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE];
#elif defined(AM_PART_APOLLO4_API)
// #### INTERNAL BEGIN ####
// This is a part of the software workaround to avoid the glitch on D1 in 1-1-4 mode by sending addr in data
// #### INTERNAL END ####
AM_SHARED_RW static uint8_t PageBuffer[AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE + 8] AM_BIT_ALIGNED(128);
#elif defined(AM_PART_APOLLO5_API)
AM_SHARED_RW static uint8_t PageBuffer[AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE + 32] __attribute__((aligned(32)));
#else
AM_SHARED_RW static uint8_t PageBuffer[AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE];
#endif

//
// Forward declarations.
//
static uint32_t am_devices_mspi_w25n02kw_command_write(void *pHandle,
                                                       uint8_t ui8Instr,
                                                       bool bSendAddr,
                                                       uint32_t ui32Addr,
                                                       uint32_t *pData,
                                                       uint32_t ui32NumBytes);
static uint32_t am_devices_mspi_w25n02kw_command_read(void *pHandle,
                                                      uint8_t ui8Instr,
                                                      bool bSendAddr,
                                                      uint32_t ui32Addr,
                                                      uint32_t *pData,
...
//*****************************************************************************
static uint8_t
w25n02kw_cmd_addr_len(uint8_t cmd)
{
    switch ( cmd )
    {
        //
        ...
        //
        case AM_DEVICES_MSPI_W25N02KW_WRITE_DISABLE:
        case AM_DEVICES_MSPI_W25N02KW_WRITE_ENABLE:
        case AM_DEVICES_MSPI_W25N02KW_RESET:
        case AM_DEVICES_MSPI_W25N02KW_READ_ID:
        return 0;

        //
        // one byte length of the command
        //
        case AM_DEVICES_MSPI_W25N02KW_SET_FEATURE:
        case AM_DEVICES_MSPI_W25N02KW_GET_FEATURE:
        return 1;

        //
        // two bytes length of the command
        //
        case AM_DEVICES_MSPI_W25N02KW_READ_BUFFER_X1:
        case AM_DEVICES_MSPI_W25N02KW_READ_BUFFER_X2:
        case AM_DEVICES_MSPI_W25N02KW_READ_BUFFER_X4:
        case AM_DEVICES_MSPI_W25N02KW_PROGRAM_LOAD_X1:
        case AM_DEVICES_MSPI_W25N02KW_PROGRAM_LOAD_X4:
        return 2;

        //
        // three bytes length of the command
        //
        case AM_DEVICES_MSPI_W25N02KW_PROGRAM_EXECUTE:
        case AM_DEVICES_MSPI_W25N02KW_BLOCK_ERASE:
        case AM_DEVICES_MSPI_W25N02KW_READ_CELL_ARRAY:
        return 3;
    }
}
#endif
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_read_status(void *pHandle, uint8_t ui8Addr, uint8_t *pu8iData)
{
    uint8_t status = AM_DEVICES_W25N02KW_OIP;

    for (uint32_t i = 0; i < AM_DEVICES_MSPI_W25N02KW_TIMEOUT; i++)
    {
        am_devices_mspi_w25n02kw_command_read(pHandle,
            AM_DEVICES_MSPI_W25N02KW_GET_FEATURE, true, (uint32_t)ui8Addr, (uint32_t *)(&status), 1);
        if ((status & AM_DEVICES_W25N02KW_OIP) != AM_DEVICES_W25N02KW_OIP)
        {
            *pu8iData = status;
            return AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS;
        }
        am_util_delay_us(10);
    }
    *pu8iData = status;
    return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
}
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_write_status(void *pHandle, uint8_t ui8Data, uint8_t ui8Addr, uint8_t *pui8Data)
{
    uint8_t status = 0;

    if (AM_HAL_STATUS_SUCCESS != am_devices_mspi_w25n02kw_command_write(pHandle,
        AM_DEVICES_MSPI_W25N02KW_SET_FEATURE, true, (uint32_t)ui8Addr, (uint32_t *)(&ui8Data), 1))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
    if (AM_HAL_STATUS_SUCCESS != am_devices_mspi_w25n02kw_command_read(pHandle,
        AM_DEVICES_MSPI_W25N02KW_GET_FEATURE, true, (uint32_t)ui8Addr, (uint32_t *)(&status), 1))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_reset(void *pHandle)
{
    uint8_t ui8PIOBuffer = 0x0;

    if (AM_HAL_STATUS_SUCCESS != am_devices_mspi_w25n02kw_command_write(pHandle, AM_DEVICES_MSPI_W25N02KW_RESET, false, 0, NULL, 0))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }

    if (AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS != am_devices_mspi_w25n02kw_read_status(pHandle, AM_DEVICES_MSPI_W25N02KW_FEATURE_STATUS, &ui8PIOBuffer))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
...
am_device_init_flash(void *pHandle)
{
    ...
    //
    if (AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS != am_devices_mspi_w25n02kw_write_status(pHandle,
        ui8FeatureData, AM_DEVICES_MSPI_W25N02KW_FEATURE_A0, &ui8FeatureStatus))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }

    ui32Status = am_devices_mspi_w25n02kw_command_read(pHandle, AM_DEVICES_MSPI_W25N02KW_READ_ID, false, 0, (uint32_t *)&ui8Response[0], 4);
    ui32Id = (ui8Response[1] << 16) | (ui8Response[2] << 8) | ui8Response[3];
    am_util_debug_printf("Device ID: %06x\n", ui32Id);
    if (AM_HAL_STATUS_SUCCESS == ui32Status)
    {
        if ((ui32Id & AM_DEVICES_MSPI_W25N02KW_ID_MASK) != AM_DEVICES_MSPI_W25N02KW_ID)
        {
    ...
    //
    // the SET FEATURES command must be issued to enable the Nand flash ECC and HOLD_DISABLE
    // setting with sending B0 feature address.
    //
    ui8FeatureData = AM_DEVICES_W25N02KW_HOLD_DISABLE | AM_DEVICES_W25N02KW_ECC_EN;
    if (AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS != am_devices_mspi_w25n02kw_write_status(pHandle,
        ui8FeatureData, AM_DEVICES_MSPI_W25N02KW_FEATURE_B0, &ui8FeatureStatus))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
...
//*****************************************************************************
static uint8_t
am_devices_mspi_w25n02kw_asize_reconfig(void * pHandle, uint8_t ui8NewLen)
{
    am_devices_mspi_w25n02kw_t *pFlash = (am_devices_mspi_w25n02kw_t *)pHandle;
...
am_devices_mspi_w25n02kw_enter_command_mode(void *pHandle)
{
    am_devices_mspi_w25n02kw_t *pFlash = (am_devices_mspi_w25n02kw_t *)pHandle;
...
//*****************************************************************************
static inline uint32_t
am_devices_mspi_w25n02kw_exit_command_mode(void *pHandle)
{
    am_devices_mspi_w25n02kw_t *pFlash = (am_devices_mspi_w25n02kw_t *)pHandle;
    if (pFlash->sCurrentSetting.eDeviceConfig != pFlash->sSerialSetting.eDeviceConfig || pFlash->sCurrentSetting.eClockFreq != pFlash->sSerialSetting.eClockFreq)
    {
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_command_write(void *pHandle, uint8_t ui8Instr, bool bSendAddr,
...
// #### INTERNAL END ####
...
#if defined(AM_PART_APOLLO4_API) || defined(AM_PART_APOLLO5_API)
    if ( (ui8Instr == AM_DEVICES_MSPI_W25N02KW_PROGRAM_EXECUTE) || (ui8Instr == AM_DEVICES_MSPI_W25N02KW_READ_CELL_ARRAY) || (ui8Instr == AM_DEVICES_MSPI_W25N02KW_BLOCK_ERASE) || (ui8Instr == AM_DEVICES_MSPI_W25N02KW_SET_FEATURE) )
    {
        // Write status/control register command byte address
        am_hal_mspi_instr_addr_t sInstAddrCfg;

        if (ui8Instr == AM_DEVICES_MSPI_W25N02KW_SET_FEATURE)
        {
    ...
    if (bSendAddr)
    {
        nlen = w25n02kw_cmd_addr_len(ui8Instr);
        olen = am_devices_mspi_w25n02kw_asize_reconfig(pFlash, nlen);
    }
#endif
    ...
    ui32Status = am_hal_mspi_blocking_transfer(pFlash->pMspiHandle, &stMSPIFlashPIOTransaction,
                                               AM_DEVICES_MSPI_W25N02KW_TIMEOUT);
    ...
    {
        am_devices_mspi_w25n02kw_asize_reconfig(pFlash, olen);
    }
#endif
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_command_read(void *pHandle, uint8_t ui8Instr, bool bSendAddr,
                                      uint32_t ui32Addr, uint32_t *pData,
                                      uint32_t ui32NumBytes)
{
    uint32_t ui32Status;
...
#if defined(AM_PART_APOLLO4_API) || defined(AM_PART_APOLLO5_API)
    if ( ui8Instr == AM_DEVICES_MSPI_W25N02KW_GET_FEATURE )
    {
...
//*****************************************************************************
static void
pfnMSPI_W25N02KW_Callback(void *pCallbackCtxt, uint32_t status)
{
#if defined(AM_PART_APOLLO5_API)
    am_hal_cachectrl_range_t sRange;
    sRange.ui32StartAddr = (uint32_t)PageBuffer;
    sRange.ui32Size = AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE + 32;
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_tx_enter_ad4_mode(void *pHandle)
{
    uint32_t ui32Status;
    am_devices_mspi_w25n02kw_t *pFlash = (am_devices_mspi_w25n02kw_t *)pHandle;
...
//*****************************************************************************
uint32_t
am_devices_mspi_w25n02kw_init(uint32_t ui32Module,
                              ...
                              void **ppMspiHandle)
{
    ...
    for (ui32Index = 0; ui32Index < AM_DEVICES_MSPI_W25N02KW_MAX_DEVICE_NUM; ui32Index++)
    {
        if (gAmW25n02kw[ui32Index].bOccupied == false)
        {
            psConfig = &gAmW25n02kw[ui32Index].sCurrentSetting;
            break;
        }
    }
    if (ui32Index == AM_DEVICES_MSPI_W25N02KW_MAX_DEVICE_NUM)
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }

    for (uint32_t i = 0; i < (sizeof(g_W25N02KW_DevConfig) / sizeof(g_W25N02KW_DevConfig[0])); i++)
    {
        if (psMSPISettings->eDeviceConfig == g_W25N02KW_DevConfig[i].eHalDeviceEnum)
        {
            *psConfig = *g_W25N02KW_DevConfig[i].psDevConfig;
    ...
    switch (psConfig->eDeviceConfig)
    {
        ...
        case AM_HAL_MSPI_FLASH_QUAD_CE0_1_1_4:
            gAmW25n02kw[ui32Index].sSerialSetting = MSPI_W25N02KW_Serial_CE0_MSPIConfig;
            ...
            if (AM_HAL_STATUS_SUCCESS != am_hal_mspi_device_configure(pMspiHandle, &MSPI_W25N02KW_Serial_CE0_MSPIConfig))
            {
                am_util_stdio_printf("Error - Failed to configure MSPI.\n");
                return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
            }
...
#if defined(W25N02KW_QUAD_CLKON4_MODE_EN)
            am_bsp_mspi_clkond4_pins_enable(ui32Module, MSPI_W25N02KW_Serial_CE0_MSPIConfig.eDeviceConfig);
#else
            am_bsp_mspi_pins_enable(ui32Module, MSPI_W25N02KW_Serial_CE0_MSPIConfig.eDeviceConfig);
#endif
            break;

        case AM_HAL_MSPI_FLASH_SERIAL_CE1:
        case AM_HAL_MSPI_FLASH_QUAD_CE1_1_1_4:
            gAmW25n02kw[ui32Index].sSerialSetting = MSPI_W25N02KW_Serial_CE1_MSPIConfig;
            if (AM_HAL_STATUS_SUCCESS != am_hal_mspi_initialize(ui32Module, &pMspiHandle))
            {
                am_util_stdio_printf("Error - Failed to initialize MSPI.\n");
                return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
            ...
            if (AM_HAL_STATUS_SUCCESS != am_hal_mspi_device_configure(pMspiHandle, &MSPI_W25N02KW_Serial_CE1_MSPIConfig))
            {
                am_util_stdio_printf("Error - Failed to configure MSPI.\n");
                return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
            }
...
#if defined(W25N02KW_QUAD_CLKON4_MODE_EN)
            am_bsp_mspi_clkond4_pins_enable(ui32Module, MSPI_W25N02KW_Serial_CE1_MSPIConfig.eDeviceConfig);
#else
            am_bsp_mspi_pins_enable(ui32Module, MSPI_W25N02KW_Serial_CE1_MSPIConfig.eDeviceConfig);
#endif
            break;
    ...
    gAmW25n02kw[ui32Index].pMspiHandle = pMspiHandle;
    gAmW25n02kw[ui32Index].ui32Module = ui32Module;

    if (AM_HAL_STATUS_SUCCESS != am_devices_mspi_w25n02kw_reset((void *)&gAmW25n02kw[ui32Index]))
    {
    ...
    //
    ui32Status = am_device_init_flash((void *)&gAmW25n02kw[ui32Index]);
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
    ...
    // Return the handle.
    //
    gAmW25n02kw[ui32Index].bOccupied = true;
    *ppMspiHandle = pMspiHandle;
    *ppHandle = (void *)&gAmW25n02kw[ui32Index];
...
//*****************************************************************************
uint32_t
am_devices_mspi_w25n02kw_deinit(void *pHandle)
{
    uint32_t ui32Status;
    am_devices_mspi_w25n02kw_t *pFlash = (am_devices_mspi_w25n02kw_t *)pHandle;
    ...
    if (AM_HAL_STATUS_SUCCESS != am_devices_mspi_w25n02kw_reset(pHandle))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
...
//*****************************************************************************
uint32_t
am_devices_mspi_w25n02kw_id(void *pHandle, uint32_t *pui32DeviceID)
{
    uint32_t ui32Status;

    am_devices_mspi_w25n02kw_enter_command_mode(pHandle);
    ...
    *pui32DeviceID = (ui8Response[1] << 16) | (ui8Response[2] << 8) | ui8Response[3];
    am_util_debug_printf("Device ID: %06x\n", *pui32DeviceID);
    if (((*pui32DeviceID & AM_DEVICES_MSPI_W25N02KW_ID_MASK) == AM_DEVICES_MSPI_W25N02KW_ID) &&
        (AM_HAL_STATUS_SUCCESS == ui32Status))
    {
        am_devices_mspi_w25n02kw_exit_command_mode(pHandle);
        return AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS;
    }
    else
    {
...
// ****************************************************************************
uint32_t
am_devices_mspi_w25n02kw_read(void *pHandle, uint32_t ui32PageNum,
                              ...
                              uint8_t *pui32EccResult)
{
    ...
    if (ui32PageNum >= AM_DEVICES_MSPI_W25N02KW_MAX_PAGES)
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
    ...
    ui32Status = am_devices_mspi_w25n02kw_command_write(pHandle, AM_DEVICES_MSPI_W25N02KW_READ_CELL_ARRAY, true, ui32PageNum, NULL, 0);
    ...
    if (AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS != am_devices_mspi_w25n02kw_read_status(pHandle, AM_DEVICES_MSPI_W25N02KW_FEATURE_STATUS, (uint8_t *)&ui32Status))
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }

    switch (ui32Status & AM_DEVICES_W25N02KW_ECCS)
    {
        case AM_DEVICES_W25N02KW_ECCS_NO_BIT_FLIPS:
            *pui32EccResult = AM_DEVICES_MSPI_W25N02KW_ECC_STATUS_NO_BIT_FLIPS;
            break;
        case AM_DEVICES_W25N02KW_ECCS_BIT_FLIPS_CORRECTED:
        case AM_DEVICES_W25N02KW_ECCS_BIT_FLIPS_CORRECTED_THR:
            *pui32EccResult = AM_DEVICES_MSPI_W25N02KW_ECC_STATUS_BIT_FLIPS_CORRECTED;
            break;
        case AM_DEVICES_W25N02KW_ECCS_BIT_FLIPS_NOT_CORRECTED:
            *pui32EccResult = AM_DEVICES_MSPI_W25N02KW_ECC_STATUS_BIT_FLIPS_NOT_CORRECTED;
            break;
        default:
    ...
    {
        ui32Status = am_devices_mspi_w25n02kw_apply_sdr_timing(pFlash, &SDRTimingConfigStored);
        if ( AM_HAL_STATUS_SUCCESS != ui32Status )
        {
            return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
        }
    ...
    Transaction.ui32TransferCount = AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE;
    // Read the whole page.
    ...
    ui32Status = am_hal_mspi_nonblocking_transfer(pFlash->pMspiHandle, &Transaction, AM_HAL_MSPI_TRANS_DMA, pfnMSPI_W25N02KW_Callback, (void *)&ui32DMAStatus);
    ...
    memcpy(pui8OobBuffer, (&PageBuffer[AM_DEVICES_MSPI_W25N02KW_PAGE_DATA_SIZE]), ui32OobLen);
...
//*****************************************************************************
uint32_t
am_devices_mspi_w25n02kw_write(void *pHandle, uint32_t ui32PageNum,
                               uint8_t *pui8DataBuffer,
                               uint32_t ui32DataLen,
                               uint8_t *pui8OobBuffer,
                               uint32_t ui32OobLen)
{
    ...
    //
    ui32Status = am_devices_mspi_w25n02kw_command_write(pHandle, AM_DEVICES_MSPI_W25N02KW_WRITE_ENABLE, false, 0, NULL, 0);
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
...
#if defined(AM_PART_APOLLO3P)
    ui32Status = am_devices_mspi_w25n02kw_tx_enter_ad4_mode(pHandle);
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
...
// #### INTERNAL END ####
        ...
        pFlash->sCurrentSetting.eDeviceConfig == AM_HAL_MSPI_FLASH_QUAD_CE1_1_1_4)
    {
        ...
        memcpy(&PageBuffer[AM_DEVICES_MSPI_W25N02KW_PAGE_DATA_SIZE + 8], pui8OobBuffer, ui32OobLen);
        ...
        // Set the transfer count in bytes.
        //
        Transaction.ui32TransferCount = AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE + 8;
    }
    else
    {
        memcpy(PageBuffer, pui8DataBuffer, ui32DataLen);
        memcpy(&PageBuffer[AM_DEVICES_MSPI_W25N02KW_PAGE_DATA_SIZE], pui8OobBuffer, ui32OobLen);
        //
        // Set the transfer count in bytes.
        //
        Transaction.ui32TransferCount = AM_DEVICES_MSPI_W25N02KW_PAGE_FULL_SIZE;
    ...
    // Wait for DMA Complete or Timeout
    for (uint32_t j = 0; j < AM_DEVICES_MSPI_W25N02KW_TIMEOUT; j++)
    {
        ...
        if (ui32Temp == AM_HAL_STATUS_SUCCESS)
        {
            ...
            ui32Status = am_devices_mspi_w25n02kw_command_write(pHandle, AM_DEVICES_MSPI_W25N02KW_PROGRAM_EXECUTE, true, ui32PageNum, NULL, 0);
            ...
            if (AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS == am_devices_mspi_w25n02kw_read_status(pHandle, AM_DEVICES_MSPI_W25N02KW_FEATURE_STATUS, (uint8_t *)&ui32Status))
            {
                if (((ui32Status & AM_DEVICES_W25N02KW_PRG_F) != AM_DEVICES_W25N02KW_PRG_F) &&
                    ((ui32Status & AM_DEVICES_W25N02KW_OIP) != AM_DEVICES_W25N02KW_OIP))
                {
                    //
                    // Send the command sequence to disable writing.
                    //
                    ui32Status = am_devices_mspi_w25n02kw_command_write(pHandle, AM_DEVICES_MSPI_W25N02KW_WRITE_DISABLE, false, 0, NULL, 0);
                    if (AM_HAL_STATUS_SUCCESS != ui32Status)
                    {
                        am_devices_mspi_w25n02kw_exit_command_mode(pHandle);
                        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
...
//*****************************************************************************
uint32_t
am_devices_mspi_w25n02kw_block_erase(void *pHandle, uint32_t ui32BlockNum)
{
    uint32_t ui32Status;

    if (ui32BlockNum >= AM_DEVICES_MSPI_W25N02KW_MAX_BLOCKS)
    {
        return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
    }
    ...
    //
    ui32Status = am_devices_mspi_w25n02kw_command_write(pHandle, AM_DEVICES_MSPI_W25N02KW_BLOCK_ERASE, true, (ui32BlockNum << 6), NULL, 0);
    ...
    for (uint32_t i = 0; i < AM_DEVICES_MSPI_W25N02KW_TIMEOUT; i++)
    {
        if (AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS == am_devices_mspi_w25n02kw_read_status(pHandle, AM_DEVICES_MSPI_W25N02KW_FEATURE_STATUS, (uint8_t *)&ui32Status))
        {
            ...
            if (((ui32Status & AM_DEVICES_W25N02KW_ERS_F) != AM_DEVICES_W25N02KW_ERS_F) &&
                ((ui32Status & AM_DEVICES_W25N02KW_OIP) != AM_DEVICES_W25N02KW_OIP))
            {
                //
                // Send the command sequence to disable writing.
...
// #### INTERNAL END ####
static uint8_t write_oob[AM_DEVICES_MSPI_W25N02KW_PAGE_OOB_SIZE] AM_BIT_ALIGNED(128);
static uint8_t read_oob[AM_DEVICES_MSPI_W25N02KW_PAGE_OOB_SIZE] AM_BIT_ALIGNED(128);
...
#else
static uint8_t write_oob[AM_DEVICES_MSPI_W25N02KW_PAGE_OOB_SIZE];
static uint8_t read_oob[AM_DEVICES_MSPI_W25N02KW_PAGE_OOB_SIZE];
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_timing_check_write(void *pHandle, uint32_t ui32PageNum,
                                            uint8_t *pui8DataBuffer,
                                            uint32_t ui32DataLen,
                                            uint8_t *pui8OobBuffer,
...
//*****************************************************************************
static uint32_t
am_devices_mspi_w25n02kw_timing_check_read(void *pHandle, uint32_t ui32PageNum,
                                           uint8_t *pui8DataBuffer,
                                           uint32_t ui32DataLen,
                                           uint8_t *pui8OobBuffer,
                                           uint32_t ui32OobLen,
...
read_from_cache(void *pHandle, uint32_t cacheadd, uint32_t *dat, uint32_t num)
{
    ...
    ui32Status = am_devices_mspi_w25n02kw_timing_check_read(pHandle, cacheadd, (uint8_t *)dat, num, &read_oob[0], 16, &ui8EccStatus, &scanCfg);
...
program_load(void *pHandle, uint32_t cacheadd, uint32_t *dat, uint32_t num)
{
    ...
    ui32Status = am_devices_mspi_w25n02kw_timing_check_write(pHandle, cacheadd, (uint8_t *)dat, num, &write_oob[0], AM_DEVICES_MSPI_W25N02KW_PAGE_OOB_SIZE, &scanCfg);
...
#if defined(AM_PART_APOLLO4) || defined(AM_PART_APOLLO4B)
const am_devices_mspi_w25n02kw_sdr_timing_config_t w25n02kw_sConfigArray[] =
{
// #### INTERNAL BEGIN ####
...
//*****************************************************************************
//
// Checks W25N02KW timing and determine a delay setting.
//
//*****************************************************************************
uint32_t
                                               ...
                                               am_devices_mspi_w25n02kw_sdr_timing_config_t *pDevSdrCfg)
{
    ...
    uint32_t ui32TestSize = sizeof(w25n02kw_sConfigArray) / sizeof(am_devices_mspi_w25n02kw_sdr_timing_config_t);
    uint32_t ui32ResultArray[2*(sizeof(w25n02kw_sConfigArray) / sizeof(am_devices_mspi_w25n02kw_sdr_timing_config_t))] = {0};
    ...
    //
    ui32Status = am_devices_mspi_w25n02kw_init(module, pDevCfg, &pDevHandle, &pHandle);
    if (AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS != ui32Status)
    {
    ...
    if ( bTimingConfigSaved )
    {
        ...
        ui32Status = am_devices_mspi_w25n02kw_block_erase(pDevHandle, FLASH_TEST_START_PAGE >> 6);
        if (AM_HAL_STATUS_SUCCESS != ui32Status)
        {
            return AM_DEVICES_MSPI_W25N02KW_STATUS_ERROR;
        }
        ...
        if ( true == flash_check(pDevHandle, FLASH_TIMING_SCAN_PAGE_NUM) )
        {
            ...
            //
            am_devices_mspi_w25n02kw_deinit(pDevHandle);
...
#if defined(AM_PART_APOLLO4) || defined(AM_PART_APOLLO4B)
        scanCfg.ui8PioTurnaround    = scanCfg.ui8XipTurnaround = w25n02kw_sConfigArray[i].ui32Turnaround;
        scanCfg.bRxNeg              = w25n02kw_sConfigArray[i].ui32Rxneg;
#else
        scanCfg.bTxNeg                = w25n02kw_sConfigArray[i].bTxNeg;
        scanCfg.bRxNeg                = w25n02kw_sConfigArray[i].bRxNeg;
        scanCfg.bRxCap                = w25n02kw_sConfigArray[i].bRxCap;
        scanCfg.ui8TxDQSDelay         = w25n02kw_sConfigArray[i].ui8TxDQSDelay;
        scanCfg.ui8Turnaround         = w25n02kw_sConfigArray[i].ui8Turnaround;
#endif
        // erase flash before flash check
        ui32Status = am_devices_mspi_w25n02kw_block_erase(pDevHandle, FLASH_TEST_START_PAGE >> 6);
        if (AM_HAL_STATUS_SUCCESS != ui32Status)
...
// ##### INTERNAL BEGIN #####
        ...
        //
#if defined(AM_PART_APOLLO4) || defined(AM_PART_APOLLO4B)
        am_util_debug_printf("\n Turnaround %d - RXNEG %d = 0x%08X\n", w25n02kw_sConfigArray[i].ui32Turnaround, w25n02kw_sConfigArray[i].ui32Rxneg, ui32ResultArray[i]);
#else
        am_util_debug_printf("\n    Setting %d = 0x%08X\n", i, ui32ResultArray[i]);
        am_util_debug_printf(" TxNeg %d, RxNeg %d, RxCap %d, TxDQSDelay %d, Turnaround %d == 0x%08X\n",
                             w25n02kw_sConfigArray[i].bTxNeg, w25n02kw_sConfigArray[i].bRxNeg, w25n02kw_sConfigArray[i].bRxCap,
                             w25n02kw_sConfigArray[i].ui8TxDQSDelay,
                             w25n02kw_sConfigArray[i].ui8Turnaround, ui32ResultArray[i]);
#endif
// ##### INTERNAL END #####
    }
#if defined(AM_PART_APOLLO4) || defined(AM_PART_APOLLO4B)
    ...
    am_util_debug_printf(" TxNeg %d, RxNeg %d, RxCap %d, TxDQSDelay %d, Turnaround %d == 0x%08X\n",
                             w25n02kw_sConfigArray[ui32MaxOnesIndex].bTxNeg, w25n02kw_sConfigArray[ui32MaxOnesIndex].bRxNeg, w25n02kw_sConfigArray[ui32MaxOnesIndex].bRxCap,
                             w25n02kw_sConfigArray[ui32MaxOnesIndex].ui8TxDQSDelay,
                             w25n02kw_sConfigArray[ui32MaxOnesIndex].ui8Turnaround, ui32ResultArray[ui32MaxOnesIndex]);
#endif
    //
    ...
    else
    {
        ...
        pDevSdrCfg->ui32Rxneg = w25n02kw_sConfigArray[ui32MaxOnesIndex].ui32Rxneg;
        pDevSdrCfg->ui32Turnaround = w25n02kw_sConfigArray[ui32MaxOnesIndex].ui32Turnaround;
#else
        pDevSdrCfg->ui8RxDQSDelay         = dqsdelay;
        pDevSdrCfg->bTxNeg                = w25n02kw_sConfigArray[ui32MaxOnesIndex].bTxNeg;
        pDevSdrCfg->bRxNeg                = w25n02kw_sConfigArray[ui32MaxOnesIndex].bRxNeg;
        pDevSdrCfg->bRxCap                = w25n02kw_sConfigArray[ui32MaxOnesIndex].bRxCap;
        pDevSdrCfg->ui8TxDQSDelay         = w25n02kw_sConfigArray[ui32MaxOnesIndex].ui8TxDQSDelay;
        pDevSdrCfg->ui8Turnaround         = w25n02kw_sConfigArray[ui32MaxOnesIndex].ui8Turnaround;
#endif
        return AM_DEVICES_MSPI_W25N02KW_STATUS_SUCCESS;
    }
}
