...
// ##### INTERNAL BEGIN #####
//
// APS6404L_QUAD_CLKON4_MODE_EN is currently only tested for the apollo4p_eb
//
// ##### INTERNAL END #####
...
//!
//! @param pMspiHandle - Pointer to the device handle.
...
//*****************************************************************************
//
//! @brief Handle am devices MSPI PSRAM aps6404l reset.
//!
//! Internal helper for the am devices MSPI PSRAM aps6404l reset operation.
//!
//! @param pMspiHandle - Pointer to the device handle.
//!
//! @return Status code from `AM_DEVICES_MSPI_PSRAM_STATUS_*` and
//!         `AM_HAL_STATUS_*`.
//
//*****************************************************************************
static uint32_t
...
//*****************************************************************************
//
//! @brief Handle PSRAM nonblocking transfer.
//!
//! Internal helper for the PSRAM nonblocking transfer operation.
//!
//! @param pPsram - Pointer to PSRAM.
//! @param bHiPrio - Boolean flag for b hi prio.
//! @param bWrite - Boolean flag for b write.
//! @param pui8Buffer - Data buffer pointer.
//! @param ui32Address - Address value.
//! @param ui32NumBytes - Value for 32 num bytes.
//! @param ui32PauseCondition - Value for 32 pause condition.
//! @param ui32StatusSetClr - Status value.
...
//    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_CE0, am_hal_gpio_pincfg_default);
//    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_CE0_PSRAM, g_AM_BSP_GPIO_MSPI0_CE0_PSRAM);
//    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI2_CE0, am_hal_gpio_pincfg_default);
//    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI2_CE0_PSRAM, g_AM_BSP_GPIO_MSPI2_CE0_PSRAM);
#endif
// #### INTERNAL END ####
...
//    am_bsp_mspi_pins_enable(ui32Module, psMSPISettings1->eDeviceConfig);
    ...
    //
    // #### INTERNAL BEGIN ####
    // DQS Config. To enable timing tuning for SDR mode
    // #### INTERNAL END ####
                                              ...
                                              uint32_t ui32NumBytes)
{
  ...
  Transaction.bSendInstr              = true;
  Transaction.ui16DeviceInstr         = AM_DEVICES_MSPI_PSRAM_QUAD_WRITE;
  Transaction.bTurnaround             = false;
  Transaction.bDCX                    = false;
  Transaction.bEnWRLatency            = false;
  ...
  while (ui32NumBytes)
  {
    if (( AM_DEVICES_MSPI_PSRAM_PAGE_SIZE - (ui32WriteAddress & (AM_DEVICES_MSPI_PSRAM_PAGE_SIZE - 1))) < ui32NumBytes)
    {
      size = AM_DEVICES_MSPI_PSRAM_PAGE_SIZE - ui32WriteAddress;
    }
    else
    {
      size = ui32NumBytes;
    ...
    // Set the address to read data from.
    Transaction.ui32DeviceAddr = ui32WriteAddress;

    // Set the target SRAM buffer address.
    Transaction.pui32Buffer = (uint32_t*)pui8TxBuffer;

    // Execute the transction over MSPI.
    ui32Status = am_hal_mspi_blocking_transfer(pPsram->pMspiHandle,
                                         &Transaction,
                                         AM_DEVICES_MSPI_PSRAM_TIMEOUT);
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
                                             ...
                                             uint32_t ui32NumBytes)
{
  ...
  while (ui32NumBytes)
  {
    if (( AM_DEVICES_MSPI_PSRAM_PAGE_SIZE - (ui32ReadAddress & (AM_DEVICES_MSPI_PSRAM_PAGE_SIZE - 1))) < ui32NumBytes)
    {
      size = AM_DEVICES_MSPI_PSRAM_PAGE_SIZE - ui32ReadAddress;
    }
    else
    {
      size = ui32NumBytes;
