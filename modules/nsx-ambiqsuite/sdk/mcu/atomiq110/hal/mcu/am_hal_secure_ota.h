...
//!
//! @addtogroup secure_ota_atomiq110 Secure OTA Functionality
...
#define AM_IMAGE_MAGIC_EXTIMG_INSTALL       0xA2
// #### INTERNAL END ####
...
typedef enum
{
    AM_PROG_INFO0_ACTIVE = 0, // program active info0 ( RRAM / OTP)
    AM_PROG_INFO0_OTP,  // program info0 opt
    AM_PROG_INFO0_RRAM, //program info0 rram
    AM_PROG_INFO0_BOTH // program both info0 opt and info0 rram
...
typedef union
{
    ...
    struct
    {
        uint32_t    offset      : 12;
        uint32_t    size        : 12;
        uint32_t    progDest    : 2; //info0 program destination ( rram, otp, active, both)
        uint32_t    rsvd        : 6;
    } info0;
// #### INTERNAL BEGIN ####
    struct
...
#define AM_IMAGE_MAGIC_AMBQ(x)   (((x) == AM_IMAGE_MAGIC_SBL)           ||  \
                                  ...
                                  ((x) == AM_IMAGE_MAGIC_EXTIMG_INSTALL))
...
// Lower 2 nibbles are used for status, hence the OTA address must be aligned to 256 bytes
#define AM_HAL_OTA_ADDR_MASK           0x2FFFFF00
// Use bit 4, 5, 6, 7 to store the return status, if failure

//
//! This batch is used as bitmask - more than one can be set - Used only for OTAPOINTER register status
//! only bits 4, 5, 6, 7( can be extended to 3, 2)
//
#define AM_HAL_OTADESC_STATUS_MASK      0x0000000F1
//
...
typedef enum
{
    ...
    AM_HAL_OTADESC_STATUS_INVALID_OTAPTR    = (int32_t)0x00000080,
    AM_HAL_OTADESC_STATUS_INVALID_OTADESC   =          0x00000040,
    AM_HAL_OTADESC_STATUS_MAX_OTA_EXCEED    =          0x00000020,
} am_hal_otadesc_status_e;

// only bits 4, 5, 6, 7( can be extended to 3, 2) can be used to store the return status, if failure
#define AM_HAL_OTA_STATUS_MASK               0x000000F0
//
//! OTA Status
//
typedef enum
{
    ...
    // Remaining values are used as numbers - only bits 4, 5, 6, 7( can be extended to 3, 2) can be used- Along with setting the FAILURE indication as value 2 in bits 0,1
    AM_HAL_OTA_STATUS_INVALID_OWNER             = (int32_t)0x00000012,
    AM_HAL_OTA_STATUS_AUTH_POLICY               =          0x00000022,
    AM_HAL_OTA_STATUS_ENC_POLICY                = (int32_t)0x00000032,
    AM_HAL_OTA_STATUS_CRC                       =          0x00000042,
    AM_HAL_OTA_STATUS_AUTH                      = (int32_t)0x00000052,
    AM_HAL_OTA_STATUS_ENC                       = (int32_t)0x00000062,
    AM_HAL_OTA_STATUS_CC                        =          0x00000072,
    AM_HAL_OTA_STATUS_MAGIC                     = (int32_t)0x00000082,
    AM_HAL_OTA_STATUS_RECURSE                   =          0x00000092,
    AM_HAL_OTA_STATUS_ERR                       = (int32_t)0x000000A2,
    AM_HAL_OTA_STATUS_INVALID_IMAGE             = (int32_t)0x000000B2,
    AM_HAL_OTA_STATUS_FAIL                      =          0x000000C2,      // Only for Patch
    AM_HAL_OTA_STATUS_VALIDATION                =          0x000000D2,
    AM_HAL_OTA_STATUS_INVALID_OPERATION         =          0x000000E2,      // Only for Wired
    AM_HAL_OTA_STATUS_INTERRUPTED               = (int32_t)0x000000F2,
} am_hal_ota_status_e;
