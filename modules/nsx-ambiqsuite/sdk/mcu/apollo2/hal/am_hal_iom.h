...
//##### INTERNAL BEGIN #####
// Unified interface for IOM operations across different modes
typedef enum
{
    AM_HAL_IOM_MODE_POLL,     //! HAL IOM Polled mode.
    AM_HAL_IOM_MODE_NB,       //! HAL IOM Non-Blocking interrupt mode.
    AM_HAL_IOM_MODE_QUEUE,    //! HAL IOM Queued interrupt mode.
    AM_HAL_IOM_MODE_MAX
} AM_HAL_IOM_MODE_E;

typedef enum
{
    AM_HAL_IOM_PROTO_SPI,
    AM_HAL_IOM_PROTO_I2C,
    AM_HAL_IOM_PROTO_MAX
} AM_HAL_IOM_PROTO_E;

typedef enum
{
    AM_HAL_IOM_OPER_WRITE,
    AM_HAL_IOM_OPER_READ,
    AM_HAL_IOM_OPER_MAX
} AM_HAL_IOM_OPER_E;
...
typedef struct
{
    AM_HAL_IOM_OPER_E       oper;
    AM_HAL_IOM_PROTO_E      proto;
    AM_HAL_IOM_MODE_E       mode;
    union
    {
        uint32_t            ui32ChipSelect; // Valid for SPI only
        uint32_t            ui32BusAddress; // Valid for I2C only
    } peerInfo;
    uint32_t                *pui32Data;
...
extern am_hal_iom_status_e am_hal_iom_operate(uint32_t ui32Module, am_hal_iom_oper_t *pOper);
//##### INTERNAL END #####
