...
static inline uint16_t cfg_vid(void) {
    ...
    return (d != NULL && d->vid != 0) ? d->vid : DEFAULT_VID;
}

static inline uint16_t cfg_pid(void) {
    const nsx_usb_device_desc_t *d = desc_cfg();
    return (d != NULL && d->pid != 0) ? d->pid : DEFAULT_PID;
}
...
const uint8_t *nsx_usb_get_webusb_url_desc(uint8_t *out_len) {
    ...
    s_webusb_url_desc[0] = (uint8_t)(3 + url_len);
    s_webusb_url_desc[1] = 3;
    s_webusb_url_desc[2] = 1;
    memcpy(&s_webusb_url_desc[3], d->webusb_url, url_len);
    s_webusb_url_len = (uint8_t)(3 + url_len);
    if (out_len != NULL) {
        *out_len = s_webusb_url_len;
    }
...
static const char *get_string(uint8_t index) {
    ...
    switch (index) {
    case 1:
        return (d != NULL && d->manufacturer != NULL) ? d->manufacturer : DEFAULT_MANUFACTURER;
    case 2:
        return (d != NULL && d->product != NULL) ? d->product : DEFAULT_PRODUCT;
    case 3:
        return (d != NULL && d->serial != NULL) ? d->serial : DEFAULT_SERIAL;
    case 4:
        return (d != NULL && d->cdc_interface != NULL) ? d->cdc_interface : DEFAULT_CDC_INTERFACE;
    case 5:
        return (d != NULL && d->vendor_interface != NULL) ? d->vendor_interface : DEFAULT_VENDOR_IFACE;
    default:
        return NULL;
    }
}
...
uint16_t const *tud_descriptor_string_cb(uint8_t index, uint16_t langid) {
    ...
    if (index == 0) {
        ...
        count = 0;
        for (; str[count] != '\0' && count < 31; count++) {
