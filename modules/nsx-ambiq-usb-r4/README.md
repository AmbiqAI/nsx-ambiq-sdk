...
AmbiqSuite R4 USB substrate for TinyUSB on Apollo4P-family SoCs.

This module stages the USB material used by the AmbiqSuite R4 SDK's Apollo4
TinyUSB examples and exposes it as an opt-in NSX module. It is intentionally
separate from the base `nsx-ambiqsuite-r4` SDK provider so applications that do
not use USB do not inherit TinyUSB or USB class-driver sources.

## Owns

- Ambiq TinyUSB device-controller port for Apollo4P-family SoCs.
- SDK-vendored TinyUSB sources needed by the Ambiq port.
- The patched Ambiq CDC/vendor class sources staged for NSX buffer ownership.
- CMake target `nsx::ambiq_usb_r4` and compatibility alias `nsx::tinyusb`.
