...
#endif // #if defined(AM_PART_APOLLO510) || defined(AM_PART_APOLLO330P_510L)
...
// #### INTERNAL BEGIN ####
#if 1   // (Remove This Conditional After Review) Avoid compiler warning "dereferencing type-punned pointer will break strict-aliasing rules"
// #### INTERNAL END ####
        void *pPeerAddr = pNvmRec->peerAddr;
        if ( (*(uint32_t*)pPeerAddr != 0xFFFFFFFF)  &&
             (*(uint32_t*)pPeerAddr != 0x00000000) )
// #### INTERNAL BEGIN ####
#else   // Original code that caused the gcc warning.
