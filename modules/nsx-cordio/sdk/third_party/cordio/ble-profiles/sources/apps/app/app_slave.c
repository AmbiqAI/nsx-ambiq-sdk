...
static void appSecPairCmpl(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  ...
  if (pCb->bondByPairing)
  {
    ...
    /* Disable bondable-mode for single-connection devices; keep it enabled for multi-connection devices. */
    if (pAppSlaveCfg == NULL || pAppSlaveCfg->connMax <= 1)
    {
                      ...
                      uint8_t *pMaxEaEvents, bool_t cfgAdvParam, uint8_t mode)
{
  ...
  if (mode == APP_MODE_AUTO_INIT)
  {
    ...
    {
      /* Allow additional bonds while connMax > 1 (e.g. two advertising sets / two centrals). */
      if (pAppSlaveCfg != NULL && pAppSlaveCfg->connMax > 1)
      {
        AppSetBondable(TRUE);
      }
