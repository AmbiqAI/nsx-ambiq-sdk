...
// #### INTERNAL BEGIN ####
// Use this macro to configure the advertising data and connection parameter for power conumption testing
#define POWER_CONSUMPTION_TEST 0
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
// This is only for power consumption test, update connection interval to 500ms, latency 0
#if POWER_CONSUMPTION_TEST
...
// #### INTERNAL BEGIN ####
// This is only for power consumption test, to fill in the maximum 31bytes advertising data
#if POWER_CONSUMPTION_TEST
...
static void fitProcMsg(fitMsg_t *pMsg)
{
  ...
  switch(pMsg->hdr.event)
  {
    ...
    case DM_CONN_OPEN_IND:
      ...
      BasProcMsg(&pMsg->hdr);
      // #### INTERNAL BEGIN ####
      // CAJ: Uncomment for automatic pairing
      // #### INTERNAL END ####
    ...
    case DM_VENDOR_SPEC_IND:
      {
        /* Vendor specific event (0xFF) received from controller */
        uint8_t *pParam = pMsg->dm.vendorSpec.param;
        uint16_t len = pMsg->dm.vendorSpec.hdr.param;   /* length stored in hdr.param */
        uint8_t status = pMsg->dm.vendorSpec.hdr.status;

        APP_TRACE_INFO2("DM_VENDOR_SPEC_IND: len=%d, status=0x%02X", len, status);

        /* Print all vendor event data */
        APP_TRACE_INFO1("Vendor Specific Event Data, len:%d", len);
        for (uint16_t i = 0; i < len; i++)
        {
          APP_TRACE_INFO2("  [%02d]: 0x%02X", i, pParam[i]);
        }
      }
      break;
