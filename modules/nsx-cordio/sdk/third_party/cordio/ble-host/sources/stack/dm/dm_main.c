...
static const uint8_t dmHciToIdTbl[] =
{
  ...
  DM_ID_ADV_PER,                                /* HCI_LE_PER_ADV_RSP_REPORT_EVT */
#endif // BT_54
};
...
static const uint16_t dmEvtCbackLen[] =
{
  ...
  sizeof(hciLePerAdvSubevtDataReqEvt_t),       /* DM_PER_ADV_SUBEVT_DATA_REQ_IND */
  sizeof(hciLePerAdvRspReportEvt_t),           /* DM_PER_ADV_RSP_REPORT_IND */
#endif // BT_54
};
...
uint16_t DmSizeOfEvt(dmEvt_t *pDmEvt)
{
  ...
  if ((pDmEvt->hdr.event >= DM_CBACK_START) && (pDmEvt->hdr.event <= DM_CBACK_END))
  {
    ...
    /* Special handling for variable-length vendor specific event */
    if (pDmEvt->hdr.event == DM_VENDOR_SPEC_IND)
    {
      /* hdr.param contains the actual vendor data length */
      len = sizeof(wsfMsgHdr_t) + pDmEvt->vendorSpec.hdr.param;
    }
  }
  else
  {
