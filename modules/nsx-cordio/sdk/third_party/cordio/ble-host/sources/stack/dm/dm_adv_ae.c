                        ...
                        uint8_t *pPeerAddr, bool_t useLegacyPdu)
{
  ...
  extAdvParam.ownAddrType = DmLlAddrType(dmAdvCb.ownAddrType[advHandle]);
  ...
  /* if local device's using a static address for this set */
  if (DM_RAND_ADDR_SA(dmCb.localAddr, dmAdvCb.ownAddrType[advHandle]))
  {
    BdaCpy(dmAdvCb.ownRandAddr[advHandle], dmCb.localAddr);
    HciLeSetAdvSetRandAddrCmd(advHandle, dmAdvCb.ownRandAddr[advHandle]);
  }
...
{
  BdaCpy(dmAdvCb.ownRandAddr[pMsg->apiSetRandAddr.advHandle], pMsg->apiSetRandAddr.addr);
...
void dmExtAdvHciHandler(hciEvt_t *pEvent)
{
  if (pEvent->hdr.event == HCI_LE_ADV_SET_TERM_CBACK_EVT)
  {
    ...
    if (pEvent->leAdvSetTerm.status == HCI_SUCCESS &&
        pEvent->leAdvSetTerm.handle != 0xFFFF)
    {
      dmConnSlaveAdvSetIdentity(pEvent->leAdvSetTerm.handle, advHandle);
    }
