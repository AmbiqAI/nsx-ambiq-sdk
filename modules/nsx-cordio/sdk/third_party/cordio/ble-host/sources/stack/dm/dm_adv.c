...
void dmAdvCbInit(uint8_t advHandle)
{
  ...
  dmAdvCb.ownAddrType[advHandle] = DM_ADDR_PUBLIC;
  BdaClr(dmAdvCb.ownRandAddr[advHandle]);
}
...
/*************************************************************************************************/
/*!
 *  \brief  Set the local address type for multiple advertising set (pairing uses per-set type).
 *          Save own address type from each advertising set separately.
 *
 *  \param  advHandle     Advertising handle.
 *  \param  addrType      Address type.
 *
 ...
 */
/*************************************************************************************************/
void DmAdvSetAddrTypeForSet(uint8_t advHandle, uint8_t addrType)
{
  WSF_ASSERT(advHandle < DM_NUM_ADV_SETS);

  WsfTaskLock();
  dmAdvCb.ownAddrType[advHandle] = addrType;
  WsfTaskUnlock();
}
