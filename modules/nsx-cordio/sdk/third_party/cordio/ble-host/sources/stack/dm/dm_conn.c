...
/* Pending (HCI connection handle -> advertising set) when LE Advertising Set Terminated
 * arrives before LE Enhanced Connection Complete allocates the connection control block. */
typedef struct
{
  uint16_t        connHandle;
  uint8_t         advHandle;
...
static dmSlaveAdvAssoc_t dmSlaveAdvAssoc[DM_CONN_MAX];

/*************************************************************************************************/
/*!
 *  \brief  Initialize slave advertising-set association table.
 *
 *  \return None.
 */
/*************************************************************************************************/
static void dmSlaveAdvAssocInit(void)
{
  uint8_t i;

  for (i = 0; i < DM_CONN_MAX; i++)
  {
    dmSlaveAdvAssoc[i].connHandle = 0xFFFF;
  }
}

/*************************************************************************************************/
/*!
 *  \brief  Consume queued advertising handle for a connection handle, if any.
 *
 *  \param  connHandle    HCI connection handle.
 *
 *  \return Advertising handle, or 0xFF if none queued.
 */
/*************************************************************************************************/
static uint8_t dmSlaveAdvAssocConsume(uint16_t connHandle)
{
  uint8_t i;

  for (i = 0; i < DM_CONN_MAX; i++)
  {
    if (dmSlaveAdvAssoc[i].connHandle == connHandle)
    {
      uint8_t advHandle = dmSlaveAdvAssoc[i].advHandle;
...
/*************************************************************************************************/
/*!
 *  \brief  Drop any queued association for a connection handle.
 *
 *  \param  connHandle    HCI connection handle.
 *
 *  \return None.
 */
/*************************************************************************************************/
static void dmSlaveAdvAssocDiscardConn(uint16_t connHandle)
{
  uint8_t i;
...
/*************************************************************************************************/
/*!
 *  \brief  Set connection local address type and address for a peripheral link from an adv set.
 *
 *  \param  pCcb          Connection control block.
 *  \param  advHandle     Advertising set index, or 0xFF to use legacy global dmCb.advAddrType.
 *
 *  \return None.
 */
/*************************************************************************************************/
static void dmConnAssignSlaveLocalIdentity(dmConnCcb_t *pCcb, uint8_t advHandle)
{
  if (advHandle < DM_NUM_ADV_SETS)
  {
    pCcb->localAddrType = dmAdvCb.ownAddrType[advHandle];
    ...
    {
      if (!BdaIsZeros(dmAdvCb.ownRandAddr[advHandle]))
      {
        BdaCpy(pCcb->localAddr, dmAdvCb.ownRandAddr[advHandle]);
      }
      else
      {
...
/*************************************************************************************************/
/*!
 *  \brief  Apply peripheral local identity from extended advertising set (SMP DHKey / pairing).
 *
 *  \param  connHandle    HCI connection handle from LE Advertising Set Terminated or connection complete.
 *  \param  advHandle     Advertising set that established the connection.
 *
 *  \return None.
 */
/*************************************************************************************************/
void dmConnSlaveAdvSetIdentity(uint16_t connHandle, uint8_t advHandle)
{
  dmConnCcb_t *pCcb;
  uint8_t      i;
  ...
  if ((pCcb = dmConnCcbByHandle(connHandle)) != NULL &&
      pCcb->role == DM_ROLE_SLAVE)
  {
    dmConnAssignSlaveLocalIdentity(pCcb, advHandle);
    return;
  }

  for (i = 0; i < DM_CONN_MAX; i++)
  {
    if (dmSlaveAdvAssoc[i].connHandle == 0xFFFF)
    {
      dmSlaveAdvAssoc[i].connHandle = connHandle;
      dmSlaveAdvAssoc[i].advHandle = advHandle;
      return;
    }
  }
}
...
void dmConnSmActConnOpened(dmConnCcb_t *pCcb, dmConnMsg_t *pMsg)
{
  ...
  /* set local address type and address of connection */
...
#if (BT_54)
    if (pMsg->hciLeConnCmpl.advHandle < DM_NUM_ADV_SETS)
    {
      advHandle = pMsg->hciLeConnCmpl.advHandle;
    }
#endif

    if (advHandle >= DM_NUM_ADV_SETS)
    {
      advHandle = dmSlaveAdvAssocConsume(pMsg->hciLeConnCmpl.handle);
    }
#if (BT_54)
    else
    {
      dmSlaveAdvAssocDiscardConn(pMsg->hciLeConnCmpl.handle);
    }
#endif
...
void dmConnReset(void)
{
  dmConnCcb_t             *pCcb = dmConnCb.ccb;
  hciEvt_t hciEvt;
  hciDisconnectCmplEvt_t  *disconnectCmpl = &hciEvt.disconnectCmpl;
  uint8_t                 i;
...
{
  dmConnCcb_t *pCcb;
  dmConnMsg_t *dmConnMsg = (dmConnMsg_t *)pEvent;
      ...
      (pEvent->hdr.event == HCI_LE_ENHANCED_CONN_CMPL_CBACK_EVT))
  {
    ...
    {
      dmConnMsg->hdr.event =  DM_CONN_MSG_HCI_LE_CONN_CMPL;
    }
    else
    {
      dmConnMsg->hdr.event = DM_CONN_MSG_HCI_LE_CONN_CMPL_FAIL;
    }
  }
  else
  {
    ...
    /* translate HCI event to state machine message */
    dmConnMsg->hdr.event = DM_CONN_HCI_EVT_2_MSG(pEvent->hdr.event);
  }
  ...
  {
    /* set conn id */
    dmConnMsg->hdr.param = pCcb->connId;

    /* execute state machine */
    dmConnSmExecute(pCcb, dmConnMsg);
  }
}
