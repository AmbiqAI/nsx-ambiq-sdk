...
#if defined(AM_PART_APOLLO3) || defined(AM_PART_APOLLO3P)
#define HCI_ACL_QUEUE_LO          4             /* Enable flow when this many buffers queued */
#else
...
// #### INTERNAL BEGIN ####
#if defined(AM_PART_APOLLO4) || defined(AM_PART_APOLLO4B) || defined(AM_PART_APOLLO4L) || defined(AM_PART_APOLLO4P)
extern bool g_HiPrioNotifyEnable;
extern uint16_t g_HiPrioNotifyHandles[MAX_HIGH_PRIORITY_ATT_HANDLE_NUM];
#endif
// #### INTERNAL END ####
/*************************************************************************************************/
/*!
...
/*************************************************************************************************/
/*!
 *  \fn     HciCheckHighPrioNotify
 *
 *  \brief  Check if the ACL packet is high priority notification/indication.
 *
 *  \param  pData    WSF buffer containing an ACL packet
 *
 *  \return true:the ACL packet is high priority notification/indication, otherwise false.
 */
/*************************************************************************************************/
static bool hciCheckHighPrioNotify(uint8_t * pData)
{
  uint16_t l2cap_cid;

  /* HCI ACL Data packet format of ATT layer:hci acl header(4bytes) +l2cap_data_length(2bytes)
      +L2CAP_CID_ATT(0x0004 2bytes)+ L2CAP pay ATT_opcode(1byte)+ATT_handle(2bytes) */
  BYTES_TO_UINT16(l2cap_cid, &pData[HCI_ACL_HDR_LEN+2]);

  if((g_HiPrioNotifyEnable) && (l2cap_cid == L2C_CID_ATT))
  {
      uint8_t att_opcode;
      uint16_t att_handle;
      att_opcode = pData[L2C_PAYLOAD_START];
      BYTES_TO_UINT16(att_handle, &pData[L2C_PAYLOAD_START+1]);

      /* check if the data is high priority ATT notification/indication */
      if((att_opcode == ATT_PDU_VALUE_NTF)||(att_opcode == ATT_PDU_VALUE_IND))
      {
        for (uint8_t i = 0; i < MAX_HIGH_PRIORITY_ATT_HANDLE_NUM; i++)
        {
            if(att_handle == g_HiPrioNotifyHandles[i])
            {
                return true;
            }
        }
...
/*************************************************************************************************/
/*!
 *  \fn     HciInsertHighPrioNotify
 *
 *  \brief  Insert the high priority notification/indication before other notifications/indications.
 *
 *  \param  pData    WSF buffer containing an ACL packet
 *
 *  \return None.
 */
/*************************************************************************************************/
static void hciInsertHighPrioNotify(uint8_t * pData)
{
  uint16_t        handle;
  hciCoreConn_t   *pConn;
  ...
  {
    wsfMsg_t * pPrev = NULL;
    wsfMsg_t * pElem = hciCoreCb.aclQueue.pHead;
    wsfMsg_t * Elem_pdata;

    /* give the pointer to the ACL data space */
    Elem_pdata = pElem;
    Elem_pdata++;

    /* skip the first acl buffer which is on-going */
    if(pConn->pTxAclPkt != NULL)
    {
      pPrev = pElem;
      pElem = pElem->pNext;
    ...
    while(pElem != NULL)
    {
      uint16_t next_l2cap_cid;
      uint8_t next_att_opcode;
      uint8_t *pbuf;

      pbuf = (uint8_t *)Elem_pdata;
      /* HCI ACL Data packet format of ATT layer:hci acl header(4bytes) +l2cap_data_length(2bytes)
      +L2CAP_CID_ATT(0x0004 2bytes)+ L2CAP pay ATT_opcode(1byte)+ATT_handle(2bytes) */
      BYTES_TO_UINT16(next_l2cap_cid, &pbuf[HCI_ACL_HDR_LEN + 2]);
      next_att_opcode = pbuf[L2C_PAYLOAD_START];

      /* check if the next element is ATT notification/indication */
      if((next_l2cap_cid == L2C_CID_ATT)&&((next_att_opcode == ATT_PDU_VALUE_NTF)||(next_att_opcode == ATT_PDU_VALUE_IND)))
      {
        /* if the next element is low priority ATT notification/indication,then insert before it */
        if(hciCheckHighPrioNotify(pbuf) == false)
        {
            break;
        }
      }
    ...
    /* insert pData into HCI ACL TX queue */
    WsfMsgInsert(&hciCoreCb.aclQueue,0,pData,pPrev);
  }
}
#endif
// #### INTERNAL END ####
...
void HciSendAclData(uint8_t *pData)
{
  ...
  if ((pConn = hciCoreConnByHandle(handle)) != NULL)
  {
    // #### INTERNAL BEGIN ####
    #if defined(AM_PART_APOLLO4) || defined(AM_PART_APOLLO4B) || defined(AM_PART_APOLLO4L) || defined(AM_PART_APOLLO4P)
    /* check if the data is high priority ATT notification and the ACL tx queue is not empty */
    if((hciCheckHighPrioNotify(pData))&&(WsfQueueCount(&hciCoreCb.aclQueue)))
    {
      hciInsertHighPrioNotify(pData);
    }
    else
    {
    #endif
