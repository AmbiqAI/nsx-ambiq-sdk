...
// #### INTERNAL END ####
    ...
    //*****************************************************************************
    //
    // Apply 32Mhz Crystal Calibration Trim.
    //
    // Please see the article "Apollo3 Blue SoC 32 MHz Crystal Calibration" at
    // support.ambiq.com for full details.  
    //
    //*****************************************************************************

    //
    // Call am_util_ble_crystal_trim_set to set the trim value for 32Mhz Crystal
    // oscillator. Default trim value is 0x400. 
    // For systems using Sitime SiT11100, default value should be 0x100.
    // AM_BSP_32M_CRYSTAL_TRIM should be of type uint32_t no greater than 0x7FF.
    //
    // am_util_ble_crystal_trim_set(BLE, AM_BSP_32M_CRYSTAL_TRIM);
...
hciDrvWrite(uint8_t type, uint16_t len, uint8_t *pData)
{
    ...
    // #### INTERNAL BEGIN ####
    // CAJ: You could argue that this whole function belongs in a critical
    // section. I think it's actually okay as long as we don't do any hci
    // writes or reads in interrupt context.
    // #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
    ...
    {
        if (BLEIFn(0)->BSTATUS_b.SPISTATUS == false)
        {
            break;
        }
        else
        {
            am_util_delay_us(HISTOGRAM_RESOLUTION);
        }
    }

    // CAJ FIXME: Infinite loop for debugging purposes only.
    // If we get here, the actual delay was longer than the histogram.
    if (BLEIFn(0)->BSTATUS_b.SPISTATUS)
    {
        while (1);
    }
    else
    {
        g_pui32StatusDelays[i]++;
    }
#else // ENABLE_HISTOGRAMS
// #### INTERNAL END ####
...
HciDrvHandler(wsfEventMask_t event, wsfMsgHdr_t *pMsg)
{
    ...
    while (ui32NumHciTransactions++ < HCI_DRV_MAX_HCI_TRANSACTIONS)
    {
        ...
        if ( BLE_IRQ_CHECK() )
        {
            ...
            {
                // #### INTERNAL BEGIN ####
                // There is no such debug event from the controller in the formal controller patch.
                if ((g_pui32ReadBuffer[0] == 4) && (g_pui32ReadBuffer[6] == 0xFF))
                {
                    uint32_t finetarget_time;
                    uint32_t sleep_duration;
                    uint32_t elt_wait_first;
                    uint32_t elt_prog;
                    uint32_t elt_cancel_first;
                    uint32_t programmed_finetarget;

                    BYTES_TO_UINT32(finetarget_time, &g_pui32ReadBuffer[10]);
                    BYTES_TO_UINT32(sleep_duration, &g_pui32ReadBuffer[14]);
                    BYTES_TO_UINT32(elt_wait_first, &g_pui32ReadBuffer[18]);
                    BYTES_TO_UINT32(elt_prog, &g_pui32ReadBuffer[22]);
                    BYTES_TO_UINT32(elt_cancel_first, &g_pui32ReadBuffer[26]);
                    BYTES_TO_UINT32(programmed_finetarget, &g_pui32ReadBuffer[30]);

                    am_util_debug_printf("finetarget_time: 0x%x\r\n", finetarget_time);
                    am_util_debug_printf("sleep_duration:  %d ms\r\n", (sleep_duration*1000)/32768);
                    am_util_debug_printf("elt_wait_first: 0x%x\r\n", elt_wait_first);
                    am_util_debug_printf("elt_prog: 0x%x\r\n", elt_prog);
                    am_util_debug_printf("elt_cancel_first: 0x%x\r\n", elt_cancel_first);
                    am_util_debug_printf("programmed_finetarget: 0x%x\r\n", programmed_finetarget);

                    // don't want to pass to host stack
                    g_ui32NumBytes = 0;
                }
                // #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
//
// Used by Huawei Xi'an to disable the controller sleep during OTA
// to work around the controller stopped responding issue HCB-74
//
// #### INTERNAL END ####
/*************************************************************************************************/
/*!
