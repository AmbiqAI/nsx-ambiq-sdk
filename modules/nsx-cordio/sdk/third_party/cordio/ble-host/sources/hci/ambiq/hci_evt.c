...
/*
 * Minimum parameter length for Command Complete/Status events.
 * Per Bluetooth Core Spec:
 * - Command Complete: Num_HCI_Command_Packets(1) + Command_Opcode(2) = 3 bytes
 * - Command Status: Status(1) + Num_HCI_Command_Packets(1) + Command_Opcode(2) = 4 bytes
 */
#define HCI_CMD_CMPL_MIN_PARAM_LEN    3
#define HCI_CMD_STATUS_MIN_PARAM_LEN  4
...
static void hciEvtParseVendorSpec(hciEvt_t *pMsg, uint8_t *p, uint8_t len)
{
  pMsg->vendorSpec.hdr.param = len;  /* store length for application */
...
static void hciEvtProcessLeExtAdvReport(uint8_t *p, uint8_t len)
{
  ...
  while (i-- > 0)
  {
    ...
    /* CVE-2024-48984: discard event if it doesn't contain enough data */
    if (ptr >= p + len)
    {
      return;
    }
  ...
  /* CVE-2024-48984: check that the last report is fully contained within the event */
  if (ptr > p + len)
  {
    return;
  }
...
void hciEvtProcessCmdCmpl(uint8_t *p, uint8_t len)
{
  ...
  /*
   * CVE-2024-48982: Validate minimum length before reading numPkts and opcode.
   * Command Complete event requires at least HCI_CMD_CMPL_MIN_PARAM_LEN bytes.
   */
  if (len < HCI_CMD_CMPL_MIN_PARAM_LEN)
  {
    HCI_TRACE_WARN1("HCI cmd cmpl too short: len=%d", len);
    return;
  }
...
void hciEvtProcessMsg(uint8_t *pEvt)
{
  ...
  uint8_t   subEvt;
  uint8_t   len;
  uint8_t   extraMsgLen = 0;  /* CVE-2024-48986: extra allocation for variable length events */
  uint8_t   cbackEvt = 0;
  hciEvt_t  *pMsg;
  uint16_t  handle;
  hciEvtCback_t cback = hciCb.evtCback;
  ...
  switch (evt)
  {
    case HCI_CMD_STATUS_EVT:
      ...
      /*
       * CVE-2024-48982: Command Status event requires HCI_CMD_STATUS_MIN_PARAM_LEN bytes.
       */
      if (len >= HCI_CMD_STATUS_MIN_PARAM_LEN)
      {
        hciEvtProcessCmdStatus(pEvt);
      }
      else
      {
        HCI_TRACE_WARN1("HCI cmd status too short: len=%d", len);
      }
      break;
    ...
    case HCI_VENDOR_SPEC_EVT:
      ...
      extraMsgLen = len;  /* CVE-2024-48986: vendor specific events have variable length */
      break;
  ...
  {
    /* allocate temp buffer */
    /* CVE-2024-48986: add extraMsgLen for variable length events (e.g., vendor specific) */
